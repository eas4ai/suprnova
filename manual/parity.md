...
| File Storage | `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` over OpenDAL | shipped | Same `put/get/delete/copy/move/exists/url` surface. Path-traversal protection built in. `Storage::register_read_through` composes two disks into a read-through disk that promotes fallback hits onto the primary. [Filesystem](filesystem.md) |
