...
| Validation | `#[derive(Validate)]` + 18 built-in rules + `Rule`/`AsyncRule` traits | shipped | `Url` uses Laravel's scheme allowlist and `Url::protocols([...])` mirrors `url:http,https`. Async rules (e.g. `Unique`) hit the DB. [Validation](validation.md) |
...
| Mail | `Mail::to(...).send(MyMail { ... }).await?` + drivers `smtp/ses/mailgun/postmark/sendgrid/resend/log/memory` | shipped | `Mailable` trait + Tera-rendered HTML/text bodies; SES sends carry `TenantName` / `ConfigurationSetName` / `ListManagementOptions`. [Mail](mail.md) |
