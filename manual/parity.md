...
| Images (`Image::read(...)->resize(...)`) | `Image::from_bytes/from_path/from_disk/from_upload/from_stream` + the same operation and terminal surface | shipped | Two drivers like Laravel's `gd`/`imagick`: `IMAGE_DRIVER=oxideav` (default, pure Rust) or `magick`. Reads and writes PNG, JPEG, WebP, GIF, BMP; AVIF output is deferred pending the in-house AV1 encoder publish. Header-checked decode limits. [Images](images.md) |
