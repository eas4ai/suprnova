...
| **Auth** | Sessões stateful, IDs de usuário opacos, múltiplos guards, provedores Eloquent + banco de dados, hashing de senha (bcrypt + argon2), macros de política, gates, verificação de email, redefinição de senha, throttling de força bruta, TOTP 2FA, remember-me, OAuth via integração torii |
...
| **Mail e notificações** | Trait `Mailable`, drivers para SMTP/SES/Mailgun/Postmark/SendGrid/Resend (mais in-memory & log para testes), `Notifiable` com canais mail/database/broadcast/webpush |
