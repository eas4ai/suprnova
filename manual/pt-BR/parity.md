...
| Validação | `#[derive(Validate)]` + 35 regras embutidas + traits `Rule`/`ValueRule`/`AsyncRule` | disponível | `Url` usa a lista de permissão de esquemas do Laravel, e `Url::protocols([...])` espelha `url:http,https`. Regras assíncronas (por exemplo, `Unique`) acessam o BD. `ArrayKeys`/`Distinct` são `ValueRule`s sobre `serde_json::Value`, correspondendo a `array:keys` e `distinct` do Laravel. `InArray` recebe a lista do outro campo diretamente, em vez da string de regra `in_array:other.*` do Laravel, e `Contains`/`DoesntContain` casam elementos de string JSON de forma exata. `Gt`/`Gte`/`Lt`/`Lte` recebem um operando `CompareWith` explícito (número literal, irmão numérico, ou irmão comparado por contagem de caracteres) em vez de inferir as quatro medidas de tamanho do Laravel; comparações de array e de arquivo não têm contrapartida. [Validação](validation.md) |
| Regra `Password` (`Password::defaults()`, `uncompromised()`) | `Password::min(n)` + builders de força (`.letters()`, `.mixed_case()`, `.numbers()`, `.symbols()`) + `.uncompromised()` | disponível | Checagem de k-anonimato do Have I Been Pwned; falha aberta em um erro de rede, casando com o `NotPwnedVerifier` do Laravel. [Validação](validation.md#password-strength) |
...
| Armazenamento de arquivos | `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` sobre o OpenDAL | disponível | Mesma superfície `put/get/delete/copy/move/exists/url`. Proteção contra path traversal embutida. Em um disco local, `put`/`write`, o writer de streaming e `copy` são todos preparados em `<root>/.suprnova-atomic/` e publicados em um único passo, e uma escrita condicional é publicada com `link(2)`, então ela continua sendo uma criação exclusiva de verdade; `append` é a única operação feita no lugar. O driver local do Laravel escreve direto no destino, onde um tamanho parcial fica observável e um crash trunca o objeto ativo. `Storage::register_read_through` compõe dois discos em um disco read-through que promove acertos do fallback para o primário, com `copy: false` para pular a promoção e `copy` / `rename` que atravessam o fallback. [Sistema de arquivos e armazenamento](filesystem.md) |
...
| Image (`Illuminate\Image`) | `Image::from_bytes/from_path/from_disk/from_upload/from_stream` + a mesma superfície de operações e terminais | disponível | Vive em `suprnova::media`. Dois drivers como o `gd`/`imagick` do Laravel: `IMAGE_DRIVER=oxideav` (padrão, Rust puro) ou `magick`. Lê e escreve PNG, JPEG, WebP, GIF, BMP; a saída AVIF fica adiada à espera da publicação do encoder AV1 interno. Limites de decodificação conferidos pelo cabeçalho. [Imagens](images.md) |
| Decodificação de HEIC no driver padrão | `IMAGE_DRIVER=magick` em um host com o delegate libheif | não, por design | O HEVC é onerado por patentes e o único decodificador em Rust puro com credibilidade é duplo AGPL/comercial, então nenhum decodificador embutido é lançado. Mesmo formato do Laravel, em que o GD não lê HEIC de jeito nenhum e o Imagick precisa do delegate compilado tanto no binário quanto na extensão do PHP. [Imagens](images.md#why-suprnova-diverges) |
...
| `Queue::forward` | `Queue::forward(from, to)` / `Queue::forward_on(from, to, connection)`, aplicado ao envelope e à lista `--queue` do worker | disponível | Somente de fila para fila: `connection` condiciona o redirecionamento em vez de selecionar um driver, e é comparada com o nome da conexão do processo, de modo que o push e a reivindicação do worker se condicionam ao mesmo valor; `to` é obrigatório onde o do Laravel é opcional. [Filas](queues.md) |
...
| Pausa de fila (`queue:pause` / `queue:resume`) | `Queue::pause`/`resume`/`pause_all`/`resume_all`/`is_paused`/`paused_queues`, apoiados em cache, com eventos `QueuePaused` / `QueueResumed` / `QueuesPaused` / `QueuesResumed` | disponível | Uma pausa por fila só afeta um worker iniciado com uma lista explícita `--queue=...`; `resume_all` não limpa uma pausa por fila. Um worker em execução também emite `WorkerQueuePaused` / `WorkerQueueResumed` uma vez por transição, e o `queue:work` imprime uma linha para cada uma; o campo `queue` deles é `Option<String>`, porque um worker iniciado sem `--queue` não tem nomes de fila para reportar sob uma pausa global. [Filas](queues.md) |
| Retentativa de comandos Redis transitórios | Comandos Redis com formato de leitura tentam de novo uma vez em uma falha de nível de conexão, com cada tentativa aguardando o orçamento de reconexão do driver; `REDIS_COMMAND_RETRIES` acrescenta mais | disponível | O `command_retries` do Laravel cobre todo comando através de um único ponto de dispatch e de uma allowlist de 60 entradas; o Suprnova tenta de novo por local de chamada, e nenhuma configuração faz uma escrita ou um pop de fila tentar de novo. [Cache](cache.md) |
| Dispatch pós-commit (`afterCommit()`) | `fn after_commit() -> bool` no `Job`, `EnvelopeOverrides::after_commit` por push, `Queue::push_after_commit` | disponível | O push inteiro espera pelo commit, eventos incluídos, e um rollback o descarta; um `push_unique` adiado ainda toma o seu lock imediatamente, então a deduplicação funciona dentro da transação. Um `DB::begin_transaction` manual nunca adia. [Fila](queues.md) |
| Conexão de fila com failover | `FailoverQueueDriver` sobre uma lista ordenada de conexões, via `QUEUE_DRIVER=failover` + `QUEUE_FAILOVER_CONNECTIONS` | disponível | As escritas caem pela lista; `pop`, os contadores e as listagens ficam na primeira conexão, então cada fallback precisa do próprio worker. O `QueueFailedOver` é disparado por borda, e o `bulk_push` cai para a seguinte por envelope, então cada um mantém o próprio atraso. [Fila](queues.md) |
| `ShouldBeUniqueUntilProcessing` | `fn unique_until_processing() -> bool` no `Job`, liberado depois da passagem do middleware e antes do handler | disponível | Liberação com escopo por dono, então uma tentativa reentregue nunca libera o lock de um dispatch mais novo. Um job que um middleware devolve para a fila mantém o seu lock. [Fila](queues.md) |
| Jobs com debounce (`#[DebounceFor]`) | `Job::debounce_for` / `max_debounce_wait` / `debounce_id`, mais `Queue::push_debounced(job, DebounceOptions)` | disponível | Métodos de trait em vez de um atributo de classe, então um erro de digitação é um erro de compilação. Todo dispatch é enfileirado e o colapso é resolvido no worker; um job que declara tanto `debounce_for` quanto `unique_id` é recusado com um `FrameworkError` onde o Laravel lança uma exceção. Chains e batches recusam de imediato um job com debounce. [Filas](queues.md) |
| Listeners em fila com debounce | `Job::debounce_for` no job do listener, ou `DebouncedListener::new(window, build).keyed_by(...)` | disponível | O Laravel coloca o atributo na classe do listener; a ponte de listener para job do Suprnova já passa por `Queue::push`, então declará-lo no job cobre o caso comum e o `DebouncedListener` cobre uma janela por registro. [Eventos](events.md) |
| Inspeção de fila (`pendingJobs` / `delayedJobs` / `reservedJobs`) | `Queue::pending_jobs(queue)` / `delayed_jobs` / `reserved_jobs`, com `Option<&str>` colapsando o par `all*Jobs()` do Laravel em uma chamada | disponível | DTO `InspectedJob` (`id`/`queue`/`name`/`attempts`/`payload`/`created_at`); o padrão da trait é um `Err` honesto em vez de uma coleção vazia; `sync`/`null` sobrescrevem com `Ok(vec![])`; o `reserved_jobs` do Redis é por consumidor. Diferente do Laravel, estes não seguem um `Queue::forward`, então eles reportam a fila literal que você nomear - que é como um backlog deixado para trás em uma fila encaminhada continua visível. [Fila](queues.md) |
| Timezone por tarefa no agendamento | `.timezone(chrono_tz::Tz)` / `.try_timezone("name")` por tarefa, padrão `Schedule::timezone`, `schedule:list --timezone` | disponível | `chrono_tz::Tz` tipado em vez da string do Laravel; o padrão para todo o agendamento é o `Schedule::timezone` em `schedule::register`, e não uma chave de configuração `app.schedule_timezone`, e uma tarefa sem fuso fixado mantém o fuso local do processo. [Agendamento de tarefas](scheduling.md) |
...
| Redefinição de senha | `PasswordReset` + transação do Magnetar para a primeira comprovação de e-mail ou fallback para um `UserProvider` verificado + e-mail de redefinição ou alteração | disponível | O Magnetar processa a primeira comprovação atômica. Aplicativos baseados em provedor podem redefinir a senha de usuários já verificados. - [Auth flows](auth-flows.md) |
...
| Autorização (Policies / Gates) | `Gate::allows/denies` + `#[policy] impl PostPolicy` + trait `Authorizable` + registro via macro + `Gate::default_denial_response` | disponível | [Autorização](authorization.md) |
...
| Opções de DSN `keepalives_*` do Postgres | Vivacidade de pool via `DB_IDLE_TIMEOUT` / `DB_MAX_LIFETIME` / `DB_ACQUIRE_TIMEOUT` / `DB_TEST_BEFORE_ACQUIRE` / `DB_PING_AFTER_IDLE` | divergente | O sqlx não expõe nenhum setter de keepalive TCP, então o Suprnova recicla e faz ping em conexões do pool no lugar. [Banco de dados](database.md#pool-liveness) |
...
| Família `whereBinary()` | `Builder::where_binary` / `or_where_binary` / `where_not_binary` / `or_where_not_binary`, e `DB::table(...).where_binary(...)` | disponível | MySQL e MariaDB emitem `= binary`; PostgreSQL e SQLite retornam um erro em vez de uma correspondência dependente de collation. [Consultas](queries.md) |
...
| Família `wherePivot` (incluindo a forma de closure) | `where_pivot` / `where_pivot_op` / `where_pivot_in` / `where_pivot_not_in` / `where_pivot_null` / `where_pivot_not_null` / `where_pivot_between` / `where_pivot_not_between` / `where_pivot_group`, mais os gêmeos `or_` | divergente | Somente leitura - um filtro de pivot nunca estreita `attach` / `detach` / `sync`, e eager loads não o carregam. [Relacionamentos](eloquent-relationships.md) |
...
| Recursos de API | `#[derive(Resource)]` + `IntoJsonResource` + `JsonApiResponse` + fieldsets + includes | disponível | Formato JSON:API + formato de recurso estilo Laravel, ambos disponíveis. Caminhos de `?include=` são limitados por `max_relationship_depth` (padrão 5), correspondendo a `JsonApiResource::$maxRelationshipDepth`. [Recursos de API](eloquent-resources.md) |
...
| `refreshForUpdate()` | `model.refresh_for_update().await?` | disponível | Recarga com `SELECT ... FOR UPDATE`; lock sem efeito no SQLite. [Bloqueio de linha](eloquent.md#row-locking) |
| `inOrderOf(col, values)` | `Builder::in_order_of(col, values)` | disponível | Ordenação por `CASE WHEN` vinculado; valores não listados ficam por último. Somente no builder tipado. [Ordenação](eloquent.md#ordering) |
| `orWhereKey` / `orWhereKeyNot` | `Builder::or_where_key(id)` / `Builder::or_where_key_not(id)` | disponível | Dobram-se na cláusula anterior como uma disjunção; aliases `or_filter_key` / `or_filter_key_not` |
...
| Middleware do protocolo Inertia (`Vary`, resposta vazia, bounce de versão) | `InertiaHeadersMiddleware` + `InertiaVersionMiddleware` + `Inertia303Middleware` - três dos middlewares conectados por `Inertia::install` (o redirecionamento de erro de validação e as páginas de erro são as duas próximas linhas) | disponível | `Vary: X-Inertia` em toda resposta; um `200` vazio numa visita Inertia vira `303` de volta; o bounce 409 refaz o flash da sessão |
...
| Páginas de erro (`Inertia::render('Error', ['status' => …])` a partir do handler de exceções) | `InertiaConfig::error_page("Error")`; `InertiaErrorPageMiddleware`, conectado por `Inertia::install` quando um componente é nomeado | disponível | Uma linha de config em vez de uma edição no handler de exceções. Um `403` / `404` / `429` / `500` numa visita Inertia ou numa navegação do navegador renderiza a página nomeada no status original, com as props `status` / `message` / `request_id`; clientes de API, `422`s, bounces de `X-Inertia-Location` e qualquer corpo que não seja um envelope de erro ficam intactos. Os três starters já trazem a página e a habilitam. [Respostas Inertia](frontend-inertia-responses.md#error-pages) |
...
| `serve` | `suprnova serve` (backend + servidor de dev Vite juntos) | disponível | [Serve](cli-serve.md). Pula o painel do Vite em um projeto `--api` em vez de se recusar a iniciar. |
...
| `db:seed` | `cargo run --bin console db:seed` (via console por app) | disponível | Seeders registrados via trait `Seeder`; uma execução direcionada imprime RUNNING / DONE com os milissegundos decorridos |
...
| Modo de manutenção (`php artisan down` / `up`) | `./app down` / `./app up` - segredo de bypass com uma expiração de 12 horas conferida pelo servidor, caminhos customizados de retry/mensagem/exceção, driver `file` ou `cache` | disponível | [Visão geral de implantação](deployment.md) |
