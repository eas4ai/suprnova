...
| Armazenamento de arquivos | `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` sobre o OpenDAL | disponível | Mesma superfície `put/get/delete/copy/move/exists/url`. Proteção contra path traversal embutida. `Storage::register_read_through` compõe dois discos em um disco read-through que promove acertos do fallback para o primário, com `copy: false` para pular a promoção e `copy` / `rename` que atravessam o fallback. [Sistema de arquivos e armazenamento](filesystem.md) |
