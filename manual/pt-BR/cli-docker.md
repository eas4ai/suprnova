...
que você pode adotar ao pé da letra ou modificar. `docker:init`
escreve um `Dockerfile` multi-estágio + `.dockerignore` para produção.
`docker:compose` escreve um `docker-compose.yml` para serviços de
