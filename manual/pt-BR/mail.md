...
`.on_queue(...)` supera tanto `Mailable::queue()` quanto qualquer `Queue::route` registrado para o job de despacho de e-mail - a mesma regra de que a "substituição por push vence" que `Queue::push_with` aplica em todo lugar. Veja [Filas](queues.md#queue-routing).
