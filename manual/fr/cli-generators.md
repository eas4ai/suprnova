...
données - et câble le nouveau module dans son `mod.rs` parent (et,
si besoin, dans `src/lib.rs` et `cmd/main.rs`). Faites-y appel quand
vous devriez sinon retaper le même boilerplate + la ligne d'import
`pub mod x;`, ce qui est le cas la plupart du temps.
