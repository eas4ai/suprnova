...
| **Authentification** | Sessions avec état, identifiants d'utilisateur opaques, guards multiples, fournisseurs Eloquent + base de données, hachage des mots de passe (bcrypt + argon2), macros de politique, portails, vérification des e-mails, réinitialisation des mots de passe, limitation des attaques par force brute, 2FA TOTP, se souvenir de moi, OAuth via intégration torii |
...
| **E-mail et notifications** | Trait `Mailable`, drivers pour SMTP/SES/Mailgun/Postmark/SendGrid/Resend (plus mémoire et journalisation pour les tests), `Notifiable` avec canaux mail/base de données/diffusion/webpush |
