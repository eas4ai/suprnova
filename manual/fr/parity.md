...
| Wrapper `TestResponse` | `suprnova::testing::TestResponse` - `assert_status` / `assert_json_path` / `assert_cookie` / `assert_session_has` fluides et consorts, tous chaînables via `&Self` | livré | [Tests HTTP](http-tests.md#fluent-response-assertions-with-testresponse) |
