...
响应携带 `Cache-Control: private, no-store` - 那是复合响应器钉在一次带槽位的
组装上的取值，而且只在整份文档被组装出来之后才会出现 - 而两个主体的文档只在
它们的岛屿标签上不同，别处都一样。“带槽位”是关键词：一个零槽位的 `Composite`
