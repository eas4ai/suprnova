...
那个问题问的是流量，而不是某一个条目。这里有两条控制台命令、六个遥测计数器、
一次有界的磁盘清扫，以及一根紧急拉杆。
...
六个封闭的计数器名称，而且它们当中没有任何一个会点名某个层级、某个提供者或某个
后端：
...
请改为断言存储里装着什么、以及被服务的那份文档是由什么构成的，也就是
...
而两个主体的文档只在
它们的岛屿标签上不同，别处都一样。响应也携带 `Cache-Control: private, no-store`，
但要按它本来的样子来读：那是整个类别的指令，在发布外壳的那次渲染上和之后每一次
组装上都同样被钉住，因为它跟着字节里装的是什么走，而不是跟着哪条路径产生了它们走。
“带槽位”在这里是关键词：一个零槽位的 `Composite`
保留的反而是这个类别的私有 `max-age`，所以那条断言适合一个里面有岛屿的路由，
而不适合一个没有岛屿的。那个测试在一次命中上断言 `renders() == before + 1`，
...
已经存下来的条目意味着什么。代码里有三个事实决定一个已存储的条目接下来会做什么，
而它们当中没有一个是“它被悄悄丢掉了”。

**一个已移动的条目不会被自动扣下。** 一致性比较（`CoherenceCheck::compare`）是
一个*双向*的不等判断，所以一个被观察到的世代与恢复后账本不同的已存储条目，无论
数字往哪个方向走都是一次移动。但一次移动并不等于拒绝服务：中间件会按至少等于其
新鲜区间的有效年龄来评估一个已移动的条目
（`framework/src/render_cache/middleware.rs` 里的 `freshness_state`），而在一个
声明了陈旧可服务窗口的路由上，这会把它落进陈旧可服务区间。**访客会被服务一次
恢复之前的那份副本，带着 `Warning`，同时重建在请求背后运行。**那就是
[RenderCache 世代](render-cache-generations.md)所描述的同一次交接，而
`an_orm_write_invalidates_the_todos_document_through_generations` 的第 4 步断言
了它。一个 `PrivateCached` 路由绝不会这么做 - 它的死亡边界就是它的新鲜边界 -
一个没有声明陈旧可服务窗口的路由也不会；两者都会在前台重建。

**一次纪元推进是按进程的。** `RenderCache::advance_epoch` 会推进账本的纪元，
然后丢弃*这个*进程的纪元租约并清空*这个*进程的 L0。它的兄弟节点两样都留着：
它们的 L0 条目，以及它们租来的那个恢复之前的纪元。每一个都会在它下一次权威读取
时得知 - 在 `CoherenceMode::Authority` 下是它紧接着的下一次命中时立即得知，在
`CoherenceMode::Lease` 下最迟是 `max_age_ms` 之后 - 而这正是
`framework/tests/render_cache/middleware.rs` 里那三个
`an_epoch_advanced_by_another_node_*` 测试所测量的。在那之前，一个兄弟节点可以
服务一个恢复之前的条目，而在一个陈旧可服务的路由上，它还能像上面那样带着
`Warning` 服务它。

**共享层级不会仅凭一次纪元变化就被清扫。** 文件层级的清扫是在一个条目的保留期
已过*或者*它的栅栏纪元 `<` 当前纪元时移除它。如果恢复这份备份把账本的纪元压到
了低于这个部署已经在其下发布过条目的那些值，那么那些条目所带的栅栏纪元现在就
*高于*当前纪元，于是那个从句不会回收它们；它们会改为等到自己的保留期走完。
数据库层级只由一次显式的 `RenderCache::sweep()` 来清扫。Redis 层级会自己回收，
但按的是 Redis 自己的节奏：每个条目哈希都存在
`<RENDER_CACHE_REDIS_PREFIX>entry:<key>` 之下（默认前缀 `suprnova_render:`），
并带着一个由该条目保留期设定的 `PEXPIRE`，所以把你声明过的最长保留期等完，就是
那个被动选项。

所以流程按顺序是这样：

1. **运行一次 `render-cache:epoch-advance`**，在恢复后的部署开始服务之前。当
   纪元单例缺失时，它会大声失败而不是报告成功，而那也正是你发现迁移没有跟着
   数据一起回来的方式。
2. **清空共享的 L1 层级。** 删掉文件层级目录里的内容、执行
   `DELETE FROM suprnova_render_entries`，或者删掉匹配 `<prefix>entry:*` 的
   Redis 键 - 看这个配置档配的是哪一个层级。请这么做，而不是等一次清扫，理由
   见上。
3. **在流量仍然关着的情况下，覆盖每一个节点的 L0。** 那次推进只清空了运行它的
   那个节点，所以在这一步做完之前，一个没被覆盖到的兄弟节点仍然可以服务一次
   恢复之前的条目 - 这就是为什么流量要一直关到这里，而不是只关到第 2 步。要么
   把其他节点重启 - 一个全新的进程有一个空的 L0，也没有租来的纪元，所以它的
   第一个请求就会读到恢复后的权威 - 要么在它们每一个上都运行
   `render-cache:epoch-advance`，那会在运行时清空各自的 L0。第二个选项会让
   账本的纪元每个节点抬一次，而这不花什么代价：纪元从那里起只会往前走，而每个
   节点最终读到的都是最后那个值。两种做法都是安全的；重启是更容易想清楚的那
   一种，而且它是唯一一种不需要做 `Lease` 模式算术的做法。

第 2 步和第 3 步正是让第 1 步变得完整而不是半截的东西。跳过它们，那么在一个带
陈旧可服务窗口的路由上，一份恢复之前的表示仍然可以被服务一次 - 正确地标着
`Warning`，并且紧接着就被重建，但确实被服务了。
...
这个缓存 - 你的会话和你的队列不在爆炸半径里。遥测是一个由六个计数器构成的封闭
