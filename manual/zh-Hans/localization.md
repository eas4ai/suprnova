...
接上之后，Inertia 的文档外壳会自己声明当前生效的语言区域：一位协商到日语的访客拿到的是 `<html lang="ja">`，采用的正是 `Locale` 渲染时那种 BCP 47 形式（`pt-BR`、`zh-Hans`）。屏幕阅读器会依据这个属性选择朗读用的语音，搜索引擎也把它读作这个页面的语言，所以一个服务于不止一种语言的应用，不再需要自己写一个中间件去重写那份已经成型的文档。
