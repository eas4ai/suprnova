...
- **Live 流现在会随着打开它的会话一起结束。** 异步成员资格在每次投递前都会重新向其 Gate 授权，却从未核对过它的会话：已注销的浏览器，或会话已被吊销的浏览器，会一直收到事件，直到流本身关闭。现在在某个节点上销毁会话时，无论是会话失效、ID 重新生成还是“在所有设备上注销”，该会话在那里打开的每个成员资格都会立即被退出；投递还会每十秒至多一次地向会话存储重新核对每个成员资格的会话，因此在另一个节点上被销毁的会话也会在这个间隔内不再收到事件。
