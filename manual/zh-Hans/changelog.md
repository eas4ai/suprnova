...
- **Live 的端点不再携带版本段。** `/__live/v1/action`、`/__live/v1/upload`、`/__live/v1/assets/*` 以及 `/__live/v1/async/*` 这一族，现在都住在去掉 `/v1` 的同样路径上：`/__live/action`、`/__live/upload`、`/__live/assets/*`、`/__live/async/*`。Suprnova 的版本就是 git 标签，浏览器运行时与框架同步发布，所以 URL 空间里的第二个版本只是许诺了一条永远不会走的演进路径。应用不受影响：这些路由由框架注册，每个 URL 都由运行时构建；手写引用 `/__live/` 路径从来就不受支持。这是在 `v2.0.1` 标签之后进入 main 的。
...
- **RenderCache 现在会尊重请求的 `Cache-Control` 指令。** 携带 `no-cache` 的请求之前会从存储中带着 `Age` 得到响应，而携带 `no-store` 的冷请求会为下一个请求填充缓存。现在 `no-store` 会同时绕过查找和发布，`no-cache` 会跳过查找，使请求由一次全新的渲染来响应。由 2026-09-13 的对抗性审计发现 (ASTRA-13)；在 `v2.0.1` 标签之后合入 main。

- **没有快照的渲染绝不会被发布。** 当快照事务无法打开时，RenderCache 会在没有读取视图的情况下渲染，并且只要代数重读一致就仍然发布，因此与并发多行写入交错的渲染可能存储两种数据库状态的混合。这样的渲染现在会被返回但不会被存储 (拒绝原因 `snapshot_unavailable`)，其重建租约也会被释放。由 2026-09-13 的对抗性审计发现 (ASTRA-08)；在 `v2.0.1` 标签之后合入 main。

- **缓存的响应会重放其 `Content-Encoding`。** 预压缩的响应体之前在存储时没有带上内容编码，因此命中时会把 gzip 字节当作纯文本返回。编码现在会与响应体一起存储，并在每次命中时重放。由 2026-09-13 的对抗性审计发现 (ASTRA-04)；在 `v2.0.1` 标签之后合入 main。

- **HEAD 请求绝不会为 GET 表示填充缓存。** 对一个在 HEAD 时不渲染任何内容的路由发起的冷 HEAD 请求，会在每个 GET 共享的键下存储一个空响应体，因此之后的 GET 都返回零字节。HEAD 未命中现在会按渲染结果返回但不存储 (拒绝原因 `head_render`)。由 2026-09-13 的对抗性审计发现 (ASTRA-03)；在 `v2.0.1` 标签之后合入 main。

- **Live 的每作用域订阅上限在并发下依然成立。** 签发过程之前会先统计某个作用域的订阅数，然后释放锁、等待授权器、随后再插入，因此一批并发请求可以全部通过计数，并在授权返回后全部被接纳，远远超过公布的每作用域 512 的上限。现在名额会在与计数相同的锁下被预留，在每条错误路径上释放，并在记录落地时交给该记录。由 2026-09-13 的对抗性审计发现 (ASTRA-07)；在 `v2.0.1` 标签之后合入 main。

- **Live 订阅在其 Gate 拒绝后即停止接收事件。** 在流的授权 Gate 被重新定义为拒绝该主体之后，已存在的异步成员关系仍会继续收到新发布的事件，因为投递只是把订阅自身保留的授权备忘与它自己进行比较。新的订阅会被正确拒绝；旧的流却不会。运行时现在会记录订阅签发给的主体，在每次投递前再次询问 Gate，并退出 Gate 不再允许的成员关系。由 2026-09-13 的对抗性审计发现 (ASTRA-01)；在 `v2.0.1` 标签之后合入 main。

- **声明了 `transaction = "required"` 的 Live 动作会在注册时被拒绝。** 宿主的事务端口是一个有文档记载的空操作，因此该策略承诺了它从未提供的原子性：每次写入都各自提交，后续阶段失败时什么也不会回滚。在端口安装真正的环境事务之前，`LiveRegistry` 现在会对这样的组件以 `RegistryErrorKind::RequiredTransactionUnsupported` 失败；没有该策略的动作照常注册。由 2026-09-13 的对抗性审计发现 (ASTRA-05)；在 `v2.0.1` 标签之后合入 main。

- **缓存路由对命名连接的读取会留在其自身的连接上。** RenderCache 未命中时会在主库的快照事务内渲染，而查询路由之前会优先选择该事务，而不是查询自身的 `on("name")` 或模型声明的连接，因此把路由加入缓存会改变其代码从哪个数据库读取。对租户库或辅助库的读取可能返回主库的行、在主库不存在的表上失败，或在有效键下发布错误内容。绑定到其他连接的读取现在即使在环境事务内也会在该连接上执行；在快照之外读取过的渲染会被返回但不会存储 (拒绝原因 `foreign_connection_read`)。由 2026-09-13 的对抗性审计发现 (ASTRA-06)；在 `v2.0.1` 标签之后合入 main。

- **数据写入与其 RenderCache 失效现在会一起提交。** 在自动提交路径上，模型保存、查询构建器写入或原始语句会先落下该行，然后在第二个事务中推进依赖代数。当第二个事务失败时，该行已持久化，API 返回错误，而依赖该行的每个缓存页面仍能通过一致性检查并继续提供写入前的内容：可见性、权限或删除的变更可能在下一次成功失效之前一直不可见。现在每个写入终端都会在一个为此打开的事务中执行行写入及其代数推进，因此二者要么一起提交，要么都不提交。在 RenderCache 判定账本表存在之后消失的账本表，现在会让写入失败，而不是带着警告被跳过。绑定到命名连接的写入 (其账本位于主库) 仍保留其独立的推进；若该推进失败，进程会停止提供已存储的条目，直到某次推进成功为止。由 2026-09-13 的对抗性审计发现 (ASTRA-10)；在 `v2.0.1` 标签之后合入 main。

- **处理器的 `Vary` 契约会在 RenderCache 存储之前得到执行。** 一个根据自有请求头改变响应体、并通过 `Vary` 声明这一点的处理器，之前会被存储在仅由路由策略构建的键之下，因此首个变体的响应体会被返回给其他所有变体，而且命中时 `Vary` 头部也不见了。凡是该头部用于选择用户、设备或实验专属内容的地方，一个请求的内容都会到达另一个请求。RenderCache 现在会在发布前解析响应的 `Vary`，当它包含 `*` 或策略未声明为键维度的字段时拒绝存储 (拒绝原因 `vary_undeclared`)。由 2026-09-13 的对抗性审计发现 (ASTRA-09)；在 `v2.0.1` 标签之后合入 main。
