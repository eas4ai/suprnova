...
- **Live 随附了组件库的基础。** 当文档通过 `with_suprnova_ui()` 选择加入时，带基础层的令牌样式表会作为运行时产物 `ui-styles` 送达；每一条规则都位于级联层 `suprnova-ui` 之内，每一个视觉值都是带浅色与深色取值的 `--sn-` 令牌，因此去掉皮肤也不会破坏任何东西。`live:add` 依据清单把组件作为 `templates/suprnova-ui/` 下的一个目录安装，保留你编辑过的文件，并接受安装在自己根目录下的第三方清单；框架在 `/suprnova-ui/<component>/<file>` 提供打包进来的样式表与脚本。表单家族以检查器现已能够展开的 Askama 宏形式提供，因此库视图能像其他视图一样通过 `live:check`。`suprnova.` 命名空间保留给组件库，注册表会拒绝来自任何其他 crate 的该命名空间。
- **Live 组件库新增浮层一族。** 工具提示、可折叠块与手风琴、弹出层、单层下拉菜单、对话框、抽屉面板和侧边抽屉与表单一族并列，通过 `live:add` 安装。每一个都在任何脚本之前通过原生原语（`details`、`popover` 属性、`dialog`）保持打开状态，打开或关闭时不发出 Live 请求，关闭时把焦点交还给触发器，并在带有 `live:preserve.self` 的稳定键之下跨越 morph 保持打开状态。
- **Live 组件库新增反馈家族和导航家族。** 提示框、骨架屏、加载指示器、进度条、空状态、toast 区域与 flash 区域；页眉栏、页脚、侧边栏、面包屑、标签页、分页和加载更多，均通过 `live:add` 安装在表单家族和浮层家族旁边。反馈呈现服务器或运行时已经持有的状态：提示框的角色跟随其变体并带有颜色之外的提示，加载呈现通过 `live:loading` 绑定并按运行时自身的时序出现，进度条是带标签和读数的原生元素，空状态的原因来自服务器状态，toast 只播报一次且不夺取焦点，而严重错误同时以提示框呈现。导航保持路由语义：带真实 URL 的锚点、来自服务器的 `aria-current`、标签页和分页显式的本地或路由模式、以 `history.replaceState` 反映且不产生历史条目的 Live 分页，以及作为按钮触发的带键追加的加载更多。视图过滤器 `live_key` 随之加入：`{% for %}` 循环内的 `live:key` 会经过它，它在渲染时强制执行检查器对字面量键所执行的键规则。
- **Live 组件库新增数据展示家族，内置集合由此完整。** 分隔线、滚动区域、比例图片、卡片、徽章、头像与头像组、列表组、描述列表和统计卡片通过 `live:add` 与先前的家族一同安装；每个组件都保持文档顺序和原生语义，每个状态都携带文本。图表在服务器上通过 `charts-rs` 渲染：`suprnova::live::charts` 中的 `render_chart` 基于有界的类型化序列把柱状或折线标记绘制为受信任的 SVG，宏在其旁放置摘要和数据表，因此没有图表脚本到达浏览器。数据表是每张表一个岛的原生表格，其排序、筛选和页码是 `#[url]` 字段，在每个动作之后反映到可分享的 URL 中。
- **Live 组件库获得了最后一个家族：live-native 家族。**
  上传组件从运行时的进度根渲染随附上传协议的状态，并且在最终确定动作之前不声称任何东西已持久化；实时信息流和通知铃铛携带运行时的流状态播报，因此降级、重连中或已关闭的流会如实说明；账户菜单是一个 details 折叠元素，带有受 CSRF 保护的注销表单和 RenderCache 下有文档记录的拼接槽。自定义元素层 (OTP 输入、日期选择器和组合框) 增强它从不替换的原生控件：每个 `sn-` 元素都是由自身 vendored 文件定义的 light DOM `HTMLElement`，且不持有表单值，因此阻止脚本后表单提交相同的值。组合框拒绝为较旧查询渲染的列表框。
- **会话阻塞把携带同一会话的请求串行化。** `SESSION_BLOCK=true` 或 `SessionConfig::block(SessionBlock::default())` 让会话中间件从加载到写入期间为会话持有一把缓存锁，路由或分组上的 `block_session` 只为那些路由启用它。持有和等待两个上限都由你决定，等待超出上限的请求以带 `Retry-After` 的 `503` 应答。没有它，同一会话上的两个并发请求以最后写入者胜出的方式写回，因此重定向设置的 flash 可能被更早开始的请求覆盖而丢失。
...
- **模板的 `live:key` 抵达 Live 运行时。** 检查器验证 `live:key`，手册也提到它，但浏览器运行时的 morph 身份、morph 控制和保留作用域只读取引擎的 `data-suprnova-live-key`，因此按检查器要求书写的带键控件没有任何效果，每个库组件都把键写了两遍。运行时现在读取 `live:key`，为其渲染的根保留引擎写法，拒绝同时携带两者且值不同的元素，组件只写 `live:key`。表单画廊的保存表单也补上了缺失的 `.prevent` 修饰符，Live 提交不再从表单自身的查询重新加载页面。
- **含有空数字输入或未选择的下拉框的 Live 表单可以提交了。** 运行时把这类控件的 null 值读成与自身不一致，从而拒绝整个提交，动作从未执行，浏览器改为原生提交表单。搜索输入的模型绑定还使用了运行时不接受的防抖值，现在改为 250 ms。
- **`live:check` 会检查视图渲染的每个元素。** 以空 call 块调用拼接 `caller()` 的宏的视图（校验摘要就是这样被调用的），在该调用之后不再向检查器提供任何内容，因此组件被判定为已证明，而视图其余部分从未被检查。空调用现在是空内容，什么都不渲染的视图会检查失败。此修复暴露了 dogfood 表单画廊所隐藏的错误。
- **声明的模型防抖时长为 100、250 或 500 毫秒。** `#[model(debounce = N)]` 曾接受 1 到 60000 ms，但检查器与浏览器运行时共享的指令文法只列出三个时长，因此其他值永远无法被模板绑定；现在会编译失败。`live:error` 也接受动作作为目标（校验摘要就是这样指定的），与运行时的解析方式一致。
- **超过七个模型字段的 Live 表单可以提交了。** 浏览器运行时会拒绝任何携带超过八个操作或模型提议的请求，以及任何携带超过十六个校验条目或八个事件的响应，尽管框架的服务器各接受 128 个；被拒绝的提交从未离开浏览器，并被报告为网络故障。浏览器现在接受服务器的数量，`live:check` 拒绝超过 127 个模型字段的 `live:submit` 表单，因上限被拒绝的请求会被报告为资源上限，并显示该动作的错误反馈。
- **Live 按 scope 的上限所接纳的每一次签发，现在都会以一个能够连接的订阅来响应。** 同一 scope 在同一毫秒内的并发签发会生成完全相同的描述符，而宿主的凭据存储只保留为一个描述符最后生成的那个密钥，于是先前的请求会在签发所执行的连接步骤返回 403 `async_authority_invalid`。另外，大约每五百次签发中会有一次返回 503 `async_unavailable`，因为它在构建信封上下文期间发布的声明与所有并发签发共用同一个槽位。描述符现在会保留每一个尚未消费的密钥，直到它被消费或过期，而构建中的声明按订阅 ID 分别保存。
- **组合框保持响应，并显示服务器返回的内容。** 没有任何选项匹配的文本会冻结页面，因为元素的观察器监视着其自身渲染会重写的属性，即使没有任何变化也是如此。元素还按子串过滤服务器返回的选项，从而隐藏了服务器搜索以其他方式匹配到的结果，例如不区分重音的匹配或按代码匹配。现在它只写入发生变化的内容，在列表框应答输入的当前文本期间显示每个选项，把对旧文本的应答保持隐藏，并且只对由 `remote=false` 选择的固定列表按键入的文本过滤。
- **库的自定义元素无论被 morph 如何移动都只绑定一次。** 组合框、OTP 输入、日期选择器和密码输入在每次连接时都会绑定监听器，而对话框、抽屉面板和侧边抽屉依据一个会被 morph 移除的属性来防护，因此被移动的元素会对一次点击响应两次，密码显示切换也会立即切回。现在每个元素在每次连接时只绑定一次，在离开文档时释放其监听器和观察器，并在原子移动期间保留它们。
- **指针位于 toast 的任何位置或焦点位于其内部时，toast 保持显示。** 区域在指针离开任一子元素时就恢复计时器，因此把指针从 toast 的文字移到其内边距上会让它在指针下超时消失，而且 toast 可能在其关闭按钮拥有焦点时被隐藏。
- **嵌套的标签页只作用于自己的标签。** 本地标签页实例会选择其下方的每个标签，并处理嵌套实例的事件，因此点击内层标签会隐藏外层面板。
- **指针移到工具提示的气泡上时，气泡保持打开。** 气泡会忽略指针，因此指针离开触发器时它就会隐藏，其文字无法阅读或选中。
- **下拉选择的下拉指示器跟随文字颜色。** 它原本是 data URI 形式的 SVG，其 `currentColor` 不会继承文档的颜色，因此在深色方案的表面上被绘制成黑色。
- **表单控件显示岛的值。** 没有任何表单宏接收值，因此以 1 挂载的数字输入会渲染为空，而未做更改的提交会提出空值。现在每个值控件接收 `value=`、`checked=` 或 `selected=`，单选组和复选框组的输入以值为键，使用户尚未发送的选择能在重新渲染后保留，`authority=` 则标记必须替换用户已输入内容的那次渲染。
- **复选框组提出已勾选值的列表。** 运行时把每个复选框都读作布尔值，因此各复选框状态不一致的组无法提交。任意大小的复选框组，以及由多个复选框绑定的任何字段，现在都按文档顺序提出已勾选的值；单个复选框仍为布尔值。
- **字段无法解码的模型提议是该字段上的校验错误。** 诸如为 `u64` 字段提出 null、为列表字段提出布尔值这样的提议会被静默丢弃：动作照常执行，响应中也不带任何校验信息。现在字段通过 `live:error` 报告该错误并保留其值，动作不会执行。
- **检查器、`live_key` 过滤器和运行时接受同一套键字母表。** 检查器和过滤器接受以 `_`、`-`、`.` 或 `:` 开头的键，而运行时会拒绝这类键，因此岛的首次 morph 会失败。现在各处的键都以 ASCII 字母或数字开头，`live:check` 让岛内的元素 ID 遵守运行时检查的规则，拒绝无效的 ID（`invalid_element_id`）和重复的 ID（`duplicate_element_id`），包括循环内的字面量 ID。
- **`live_key_digest` 可以为任意值生成键。** 对于键字母表之外的值，`live_key` 会使岛的渲染失败，因此以电子邮件地址为键的行会让该岛对每个查看者都失败。新的过滤器把任意值转换为稳定的键，同一个值总是得到同一个键。
- **`live:check` 把循环或 match 绑定当作该绑定本身来检查。** 在宏体内，由 `for`、`match` 分支或 `if let` 绑定的名称会被当作同名的宏参数来检查，因此字面量实参会证明一条实际由循环自身的值渲染的指令。
- **`render_chart` 对超过 1e9 的值返回错误。** 渲染器的坐标轴运算大约从 1e12 开始溢出并 panic；现在绝对值超过 1e9 的值是输入错误。
- **无法读取其 vendored 组件的应用会拒绝启动。** `try_live_ui_assets()` 在每次请求时读取应用基础路径下的 `templates/suprnova-ui/`，因此在别处启动的应用（例如从只包含二进制文件的容器镜像启动）会对每个组件样式表和脚本返回 404，却仍能顺利启动。现在安装该路由会失败并指出该目录。
- **`live:add` 会替换从未被编辑过的文件。** 它此前只比较字节，因此库更新后会保留每个已安装的文件，并报告其已在本地编辑。现在它在组件旁记录每个文件的摘要，并替换字节仍与记录一致的文件；被编辑过的文件，或没有任何记录能担保的文件，会被保留并报告。
- **`live:add --manifest` 拒绝本身是符号链接的文件。** 此前每个第三方文件都会跟随链接读取，因此组件可以把任意可读文件的字节安装为模板。被列出的文件必须是清单目录内的普通文件。
- **渲染替换过的模型编辑，在再次输入时也会被发送。** 浏览器会把每次编辑与它最后一次提出的值相比较，因此在一个被拒绝的值和替换它的渲染（例如重置）之后，再次输入同一个值不会发送任何内容：控件会无误地显示该值，而岛却持有另一个值。现在，岛所应用的渲染成为下一次编辑比较的基准，字段的脏状态也与该渲染赋给其控件的值相比较。
- **工具提示现在可以在其显示的位置被关闭。** 覆盖内容的气泡此前只能通过移开指针或焦点来关闭，而这正是 WCAG 2.2 成功标准 1.4.13 所不允许的。Escape 键现在无需移动两者中的任何一个即可将其关闭，再次悬停或聚焦该触发器会让它重新显示。即使页面不加载任何脚本，气泡依然照常显示；新的 `sn-tooltip` 元素独自承担这一关闭动作，`live:add tooltip` 会安装它。

### 安全

- **锁文件纳入了 rustls 的 TLS 1.3 握手修复。** `rustls` 0.23.45 取代了 0.23.40，后者会跨加密级别边界接受 TLS 1.3 握手消息 （RUSTSEC-2026-0285，于发布当天公布）；`aws-lc-rs` 和 `rustls-webpki` 随之更新。框架中的每一个 HTTPS 客户端，从 HTTP 门面到 Qdrant 和支付适配器，都会解析到修复后的版本。
- **Live 流现在会随着打开它的会话一起结束。** 异步成员资格在每次投递前都会重新向其 Gate 授权，却从未核对过它的会话：已注销的浏览器，或会话已被吊销的浏览器，会一直收到事件，直到流本身关闭。现在在某个节点上销毁会话时，无论是普通注销、会话失效、ID 重新生成还是“在所有设备上注销”，该会话在那里打开的每个成员资格都会立即被退出；投递还会每十秒至多一次地向会话存储重新核对每个成员资格的会话，因此在另一个节点上被销毁的会话也会在这个间隔内不再收到事件。
- **RenderCache 现在会尊重请求的 `Cache-Control` 指令。** 携带 `no-cache` 的请求之前会从存储中带着 `Age` 得到响应，而携带 `no-store` 的冷请求会为下一个请求填充缓存。现在 `no-store` 会同时绕过查找和发布，`no-cache` 会跳过查找，使请求由一次全新的渲染来响应。由 2026-09-13 的对抗性审计发现 (ASTRA-13)。
- **没有快照的渲染绝不会被发布。** 当快照事务无法打开时，RenderCache 会在没有读取视图的情况下渲染，并且只要代数重读一致就仍然发布，因此与并发多行写入交错的渲染可能存储两种数据库状态的混合。这样的渲染现在会被返回但不会被存储 （拒绝原因 `snapshot_unavailable`），其重建租约也会被释放。由 2026-09-13 的对抗性审计发现 (ASTRA-08)。
- **缓存的响应会重放其 `Content-Encoding`。** 预压缩的响应体之前在存储时没有带上内容编码，因此命中时会把 gzip 字节当作纯文本返回。编码现在会与响应体一起存储，并在每次命中时重放。由 2026-09-13 的对抗性审计发现 (ASTRA-04)。
- **HEAD 请求绝不会为 GET 表示填充缓存。** 对一个在 HEAD 时不渲染任何内容的路由发起的冷 HEAD 请求，会在每个 GET 共享的键下存储一个空响应体，因此之后的 GET 都返回零字节。HEAD 未命中现在会按渲染结果返回但不存储 （拒绝原因 `head_render`）。由 2026-09-13 的对抗性审计发现 (ASTRA-03)。
- **Live 的每作用域订阅上限在并发下依然成立。** 签发过程之前会先统计某个作用域的订阅数，然后释放锁、等待授权器、随后再插入，因此一批并发请求可以全部通过计数，并在授权返回后全部被接纳，远远超过公布的每作用域 512 的上限。现在名额会在与计数相同的锁下被预留，在每条错误路径上释放，并在记录落地时交给该记录。由 2026-09-13 的对抗性审计发现 (ASTRA-07)。
- **Live 订阅在其 Gate 拒绝后即停止接收事件。** 在流的授权 Gate 被重新定义为拒绝该主体之后，已存在的异步成员关系仍会继续收到新发布的事件，因为投递只是把订阅自身保留的授权备忘与它自己进行比较。新的订阅会被正确拒绝；旧的流却不会。运行时现在会记录订阅签发给的主体，在每次投递前再次询问 Gate，并退出 Gate 不再允许的成员关系。由 2026-09-13 的对抗性审计发现 (ASTRA-01)。
- **声明了 `transaction = "required"` 的 Live 动作会在注册时被拒绝。** 宿主的事务端口是一个有文档记载的空操作，因此该策略承诺了它从未提供的原子性：每次写入都各自提交，后续阶段失败时什么也不会回滚。在端口安装真正的环境事务之前，`LiveRegistry` 现在会对这样的组件以 `RegistryErrorKind::RequiredTransactionUnsupported` 失败；没有该策略的动作照常注册。由 2026-09-13 的对抗性审计发现 (ASTRA-05)。
- **缓存路由对命名连接的读取会留在其自身的连接上。** RenderCache 未命中时会在主库的快照事务内渲染，而查询路由之前会优先选择该事务，而不是查询自身的 `on("name")` 或模型声明的连接，因此把路由加入缓存会改变其代码从哪个数据库读取。对租户库或辅助库的读取可能返回主库的行、在主库不存在的表上失败，或在有效键下发布错误内容。绑定到其他连接的读取现在即使在环境事务内也会在该连接上执行；在快照之外读取过的渲染会被返回但不会存储 （拒绝原因 `foreign_connection_read`）。由 2026-09-13 的对抗性审计发现 (ASTRA-06)。
- **数据写入与其 RenderCache 失效现在会一起提交。** 在自动提交路径上，模型保存、查询构建器写入或原始语句会先落下该行，然后在第二个事务中推进依赖代数。当第二个事务失败时，该行已持久化，API 返回错误，而依赖该行的每个缓存页面仍能通过一致性检查并继续提供写入前的内容：可见性、权限或删除的变更可能在下一次成功失效之前一直不可见。现在每个写入终端都会在一个为此打开的事务中执行行写入及其代数推进，因此二者要么一起提交，要么都不提交。在 RenderCache 判定账本表存在之后消失的账本表，现在会让写入失败，而不是带着警告被跳过。绑定到命名连接的写入 （其账本位于主库） 仍保留其独立的推进；若该推进失败，进程会停止提供已存储的条目，直到某次推进成功为止。由 2026-09-13 的对抗性审计发现 (ASTRA-10)。
- **处理程序的 `Vary` 契约会在 RenderCache 存储之前得到执行。** 一个根据自有请求头改变响应体、并通过 `Vary` 声明这一点的处理程序，之前会被存储在仅由路由策略构建的键之下，因此首个变体的响应体会被返回给其他所有变体，而且命中时 `Vary` 头部也不见了。凡是该头部用于选择用户、设备或实验专属内容的地方，一个请求的内容都会到达另一个请求。RenderCache 现在会在发布前解析响应的 `Vary`，当它包含 `*` 或策略未声明为键维度的字段时拒绝存储 （拒绝原因 `vary_undeclared`）。由 2026-09-13 的对抗性审计发现 (ASTRA-09)。
- **RenderCache 现在会尊重处理程序的 `Cache-Control: no-store`。** 一个加入缓存的路由之前会存储处理程序声明了 `no-store` 的响应，并以策略自身的 `public, max-age=60, s-maxage=60` 重放它，因此处理程序的“不要存储”在服务端被忽略，并且对下游的每个浏览器和代理都被改写。资格检查现在会读取响应的 `Cache-Control`，遇到 `no-store` 令牌即拒绝存储 （拒绝原因 `no_store_directive`），被拒绝的响应会按处理程序构建的原样发出。由 2026-09-13 的对抗性审计发现 (ASTRA-02)。
- **每个响应独有的 CSP nonce 不会再从缓存中重放。** 一个在每次渲染时生成 nonce 并在 `Content-Security-Policy` 中声明它的公开页面，之前会被作为普通的完整条目存储，因此之后的每次命中都会在头部和内联脚本中携带首次渲染的 nonce。nonce 是单个响应中内联脚本的授权令牌；重放它意味着任何能获取该页面的人都能读到这个令牌。RenderCache 现在会拒绝存储这样的响应 （拒绝原因 `nonce_source_policy`）；基于哈希的策略照常缓存，拼接的 Live 文档也继续在每次命中时签发新的 nonce。由 2026-09-13 的对抗性审计发现 (ASTRA-12)。
- **缓存的响应会保留其隔离与执行相关的头部。** RenderCache 之前不会存储响应的 `Content-Disposition`、`Cross-Origin-Opener-Policy`、`Cross-Origin-Embedder-Policy`、`Cross-Origin-Resource-Policy`、`Permissions-Policy` 和 `X-Frame-Options`，因此缓存命中会返回同样的字节却不带这些头部。一次在首个请求中作为附件下载的 HTML 导出，在第二次请求时会在应用的源之下内联渲染，其中携带的任何标记都会以同源权限执行。这六个头部现在会从存储的表示中逐字节重放。由 2026-09-13 的对抗性审计发现 (ASTRA-11)。
- **锁文件甩掉了一个不健全的发布版本和三个已撤回的发布版本。** `event-listener` 5.4.2 取代了 5.4.1：后者栈上分配的监听器被无条件标记为 `Send`/`Sync`，让一个 `!Send` 的标签能够在安全代码中跨越线程（RUSTSEC-2026-0221）；Suprnova 的依赖只使用不带标签的事件，所以这条不健全的路径在这里从未被走到。`spin` 0.9.9 和 0.10.1 以及 `chacha20` 0.10.2 取代了被发布者撤回的版本，`concurrent-queue` 则彻底离开依赖树。
