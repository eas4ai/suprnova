...
| `InertiaHeadersMiddleware` / `InertiaVersionMiddleware` / `Inertia303Middleware` / `InertiaValidationRedirectMiddleware` / `EncryptHistoryMiddleware` | Inertia 协议：每一个响应上的 `Vary: X-Inertia` 以及空 200 回跳重定向；资产版本 409 弹回；非 GET 重定向上的 302→303；一次 Inertia 访问中的 422 会变成带着已闪存错误的 303 回跳；历史加密。`Inertia::install` 会注册前四个；`EncryptHistoryMiddleware` 则单独选择性加入。参见 [Inertia 响应](frontend-inertia-responses.md#bootstrap-inertia-install) |
