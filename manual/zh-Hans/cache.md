...
- 亚秒级的 `Duration` 在 `EX` 下会被截断成 `0 seconds`，而 Redis 会拒绝这个值（`SET … EX 0`），或者更糟，把它解读成“删除这个键”（`EXPIRE key 0`）。
- `Duration::ZERO` 在调用之前会被夹到 1 毫秒，所以这两条拒绝路径，用户代码都走不到。

### 瞬时命令的重试

一个被断开的套接字，以前会让当时正在飞行中的那一次 `Cache::get` 失败。Redis 连接管理器会自己重连，但撞上那个死套接字的命令，仍然会把它的错误返回给您。

读形状的命令现在会重试一次：`GET`、`EXISTS`，以及 `Cache::flush` 和 `Cache::flush_tags` 背后的那些 `SCAN` / `SSCAN` 分页。队列驱动程序的 `XLEN`、`ZCARD` 和 `XPENDING` 读取，以及限流器的 `Retry-After` 计算，也以同样的方式重试。设置 `REDIS_COMMAND_RETRIES`，可以在内置的这一次之上再加上更多重试。

给这次重试做预算时，要按秒来算，而不是按它之前那 50 毫秒的停顿来算。一旦一个连接断掉了，下一次尝试就得先等到替换连接就位，才能发出任何东西，所以它要付掉这个驱动程序的整个连接预算，然后再付一次它的响应超时：

- 缓存驱动程序允许最多 3 次连接重试，彼此间隔至多 500 毫秒，每一次都受一个 2 秒的连接超时约束，另有一个 5 秒的响应超时。
- 队列驱动程序和限流驱动程序采用 redis-rs 的默认值：最多 6 次连接重试，指数退避从 100 毫秒起、不设上限，每一次都受一个 1 秒的连接超时约束，另有一个 500 毫秒的响应超时。

`REDIS_COMMAND_RETRIES` 被夹在 10，而这个夹取约束的是尝试次数，不是秒数：在最大值下，单单一次读取就会做出 12 次尝试，面对一个已经宕掉的 Redis，这在一次调用上就是几十秒到几分钟。一条超时的命令和一个被断开的套接字一样，都算作瞬时故障，所以一个仅仅是慢的 Redis，会让每一次被包起来的读取都发出多达这么多条命令，而不是一条。只有在调用方等得起的地方，才把这个设置调高。

写操作在任何设置下都绝不重试。一个瞬时错误意味着连接失败了，而不是服务器拒绝了这条命令 - 服务器可能已经把它跑过了 - 所以重试一次 `SET`、一次 `INCR`、一次锁获取、一次限流计数，或者一次队列弹出，都有二次执行的风险。这些命令会把失败浮现给您，而您做出的那个重试决定，才是知情的那一个。

### 为什么 Suprnova 有所不同

Laravel 的 `command_retries` 配置项会为每一条 Redis 命令都提高重试预算，因为它的 `command()` 方法是一个单一的收口点，知道自己正在跑的是哪条命令，并且会去查一张 60 条目的只读允许列表。Suprnova 的驱动程序直接调用带类型的命令，所以那张允许列表变成了一个逐调用点的决定，而 `REDIS_COMMAND_RETRIES` 只能把重试加深到那些本来就可以安全重复的命令上。没有任何设置能让一次队列弹出去重试。
## 测试
...
框架自己的 Redis 测试都标了 `#[ignore]`，所以 `cargo test` 从来不需要一台服务器。用 `-- --ignored` 来跑它们，并把它们指向一个实例：

- `cache_redis_integration` 读取 `CACHE_REDIS_TEST_URL`，回退到 `REDIS_URL`，再回退到 `redis://127.0.0.1:6379`。每个测试都把自己限定在一个唯一的键前缀里，所以拿一台共享的开发用 Redis 来跑也是安全的。
- `cache_redis_retry` 覆盖的是瞬时命令的重试，它明确要求 `CACHE_REDIS_TEST_URL`，没有回退。它会发出 `CLIENT KILL TYPE normal`，那会断开这个实例上其他每一个客户端，所以必须给它一台用完即弃的服务器。这个变量没有设置时，它会打印一行跳过信息，然后不做连接就通过。
## 模式
