...
`WebPushClient::new` 会应用一个逐请求 30 秒的超时。如果您需要一个不同的传输策略 - 企业代理、固定的 TLS、更短的超时 - 就把一个
`reqwest::ClientBuilder` 传给 `WebPushClient::with_client_builder`。构建器的所有选项都会被采纳，但重定向策略会被强制禁用：一个已验证的端点即使返回 3xx，也绝不能把 POST 转发到未经验证的 URL，因此库不会接受调用方的重定向设置。
...
`WebPushClient::with_client` 接收一个已经构建好的客户端，库无法检查它的重定向策略。在默认的 `Strict` 策略下，这类传输的发送会在任何 I/O 之前被拒绝 - 请切换到 `with_client_builder`，或者在确知客户端不会跟随重定向时，用 `.allow_unconfined_redirects()` 显式接受该风险。
