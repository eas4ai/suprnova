...
| Inertia 协议中间件（`Vary`、空响应、版本弹回） | `InertiaHeadersMiddleware` + `InertiaVersionMiddleware` + `Inertia303Middleware` - `Inertia::install` 接入的中间件中的三个（验证错误重定向和错误页面是接下来的两行） | 已实现 | 每个响应均有 `Vary: X-Inertia`；Inertia 访问的空 `200` 会变成回跳的 `303`；409 弹回会重新 flash 会话 |
...
| 错误页面（异常处理程序里的 `Inertia::render('Error', ['status' => …])`） | `InertiaConfig::error_page("Error")`；`InertiaErrorPageMiddleware`，在点名了一个组件时由 `Inertia::install` 接入 | 已实现 | 一行配置，而不是去改异常处理程序。一次 Inertia 访问或一次浏览器导航上的 `403` / `404` / `429` / `500`，会以原始状态码渲染那个被点名的页面，带上 `status` / `message` / `request_id` 这几个 prop；API 客户端、`422`、`X-Inertia-Location` 弹回，以及任何不是错误 envelope 的响应体，都不受影响。三个起始套件都带上了这个页面并启用了它。[Inertia 响应](frontend-inertia-responses.md#error-pages) |
