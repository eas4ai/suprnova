...
- **你通过 `DB::select`、`DB::select_one`、`DB::scalar` 或
  ...
  路径解析的。RBAC 角色与权限检查也是通过那些原始语句读取的，
  所以一个执行了其中之一的已缓存路由永远不会被存储。
- **这次写入是由一个队列工作进程、一个计划任务或一个控制台
  命令做的。** 只有运行了 `RenderCache::install` 的那个
  进程（也就是服务器）才会推进世代；来自其他进程的写入不会，
  在那些进程里调用 `RenderCache::bump_permission_version()`
  同样什么都不会发生。依赖这样一次写入的页面，只会在它的新鲜
  度窗口内保持最新；请在一个会改变已缓存页面所显示内容的作业
  之后，运行 `render-cache:epoch-advance`。

在 PostgreSQL 上，渲染运行在一个 `REPEATABLE READ` 事务中，
好让它读到的内容与它记录下的那些世代保持一致；一个已缓存
路由的处理程序如果更新了某一行，而这一行在渲染开始之后被
另一个事务改动过，就会遇到一次序列化失败。被缓存的路由都
是读路径。
...
- **`RenderCache::bump_permission_version().await?`** - 每当
  应用中的某个动作改变了一个已登录用户被允许做什么（一次角色
  变更、一次权限授予或撤销）时，就调用它。它会推进一个持久化
  的世代，每一个按访客建键的渲染都会关注这个世代，因此它能
  挺过一次重启，并且在角色变更运行在某个事务中时，会加入那个
  事务。不这样做的话，一个权限刚刚发生变化的用户，仍然会命中
  在其先前权限集合下缓存的内容。
