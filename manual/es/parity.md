...
| Almacenamiento de archivos | `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` sobre OpenDAL | disponible | La misma superficie `put/get/delete/copy/move/exists/url`. Protección contra path traversal incorporada. `Storage::register_read_through` compone dos discos en un disco read-through que promueve al primario los aciertos del fallback, con `copy: false` para saltarse la promoción, y `copy` / `rename` que abarcan el fallback. [Sistema de archivos](filesystem.md) |
