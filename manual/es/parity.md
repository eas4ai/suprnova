...
| Validación | `#[derive(Validate)]` + 35 reglas integradas + los traits `Rule`/`ValueRule`/`AsyncRule` | disponible | `Url` usa la lista de permitidos de esquemas de Laravel y `Url::protocols([...])` refleja `url:http,https`. Las reglas asíncronas (por ejemplo, `Unique`) van a la base de datos. `ArrayKeys` y `Distinct` son `ValueRule`s sobre `serde_json::Value`, y se corresponden con `array:keys` y `distinct` de Laravel. `InArray` toma directamente la lista del otro campo, en lugar de la cadena de regla `in_array:other.*` de Laravel, y `Contains` / `DoesntContain` casan exactamente los elementos JSON de tipo cadena. `Gt` / `Gte` / `Lt` / `Lte` toman un operando `CompareWith` explícito (un número literal, un hermano numérico, o un hermano comparado por número de caracteres) en lugar de inferir las cuatro medidas de tamaño de Laravel; las comparaciones de arrays y de ficheros no tienen contraparte. [Validación](validation.md) |
...
| Almacenamiento de archivos | `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` sobre OpenDAL | disponible | La misma superficie `put/get/delete/copy/move/exists/url`. Protección contra path traversal incorporada. En un disco local, `put`/`write`, el escritor en streaming y `copy` se preparan todos bajo `<root>/.suprnova-atomic/` y se publican en un solo paso, y una escritura condicional se publica con `link(2)` para que siga siendo una creación exclusiva de verdad; `append` es la única operación en el sitio. El driver local de Laravel escribe directamente sobre el destino, donde una longitud parcial es observable y una caída trunca el objeto vivo. `Storage::register_read_through` compone dos discos en un disco read-through que promueve al primario los aciertos del fallback, con `copy: false` para saltarse la promoción, y `copy` / `rename` que abarcan el fallback. [Sistema de archivos](filesystem.md) |
...
| `Queue::forward` | `Queue::forward(from, to)` / `Queue::forward_on(from, to, connection)`, aplicado al sobre y a la lista `--queue` del worker | disponible | Solo de cola a cola: `connection` condiciona la redirección en lugar de seleccionar un driver, y se compara con el nombre de conexión del proceso para que el push y la reclamación del worker se condicionen por el mismo valor; `to` es obligatorio, mientras que el de Laravel es opcional. [Cola](queues.md) |
...
| Pausar colas (`queue:pause` / `queue:resume`) | `Queue::pause`/`resume`/`pause_all`/`resume_all`/`is_paused`/`paused_queues`, respaldados por caché, con los eventos `QueuePaused` / `QueueResumed` / `QueuesPaused` / `QueuesResumed` | disponible | Una pausa por cola solo surte efecto en un worker arrancado con una lista `--queue=...` explícita; `resume_all` no limpia una pausa por cola. Un worker en marcha emite además `WorkerQueuePaused` / `WorkerQueueResumed` una vez por transición y `queue:work` imprime una línea por cada uno; su campo `queue` es `Option<String>`, porque un worker arrancado sin `--queue` no tiene nombres de cola de los que informar bajo una pausa global. [Cola](queues.md) |
| Reintentar comandos de Redis transitorios | Los comandos de Redis con forma de lectura reintentan una vez ante un fallo a nivel de conexión, y cada intento espera al presupuesto de reconexión del driver; `REDIS_COMMAND_RETRIES` añade más | disponible | El `command_retries` de Laravel cubre todos los comandos a través de un único punto de despacho y una lista de permitidos de 60 entradas; Suprnova reintenta por punto de llamada, y ningún ajuste hace que se reintente una escritura ni que se saque un job de la cola. [Caché](cache.md) |
...
| Jobs con antirrebote (`#[DebounceFor]`) | `Job::debounce_for` / `max_debounce_wait` / `debounce_id`, más `Queue::push_debounced(job, DebounceOptions)` | disponible | Métodos de trait en lugar de un atributo de clase, así que una errata es un error de compilación. Cada despacho se encola y el colapso se resuelve en el worker; un job que declara a la vez `debounce_for` y `unique_id` se rechaza con un `FrameworkError` allí donde Laravel lanza una excepción. Las cadenas y los lotes rechazan de plano un job con antirrebote. [Cola](queues.md) |
| Oyentes encolados con antirrebote | `Job::debounce_for` en el job del oyente, o `DebouncedListener::new(window, build).keyed_by(...)` | disponible | Laravel pone el atributo en la clase del oyente; el puente de oyente a job de Suprnova ya pasa por `Queue::push`, así que declararlo en el job cubre el caso común y `DebouncedListener` cubre una ventana por registro. [Eventos](events.md) |
| Inspección de la cola (`pendingJobs` / `delayedJobs` / `reservedJobs`) | `Queue::pending_jobs(queue)` / `delayed_jobs` / `reserved_jobs`, con un `Option<&str>` que colapsa en una sola llamada el gemelo `all*Jobs()` de Laravel | disponible | DTO `InspectedJob` (`id`/`queue`/`name`/`attempts`/`payload`/`created_at`); el valor por defecto del trait es un `Err` honesto en lugar de una colección vacía; `sync` y `null` lo sobrescriben con `Ok(vec![])`; el `reserved_jobs` de Redis es por consumidor. A diferencia de Laravel, estas no siguen un `Queue::forward`, así que informan de la cola literal que nombras - que es como se mantiene visible el trabajo acumulado que queda atrás en una cola reenviada. [Cola](queues.md) |
...
| Opciones de DSN `keepalives_*` de Postgres | Vitalidad del pool con `DB_IDLE_TIMEOUT` / `DB_MAX_LIFETIME` / `DB_ACQUIRE_TIMEOUT` / `DB_TEST_BEFORE_ACQUIRE` / `DB_PING_AFTER_IDLE` | diverge | sqlx no expone ningún setter de keepalive de TCP, así que Suprnova recicla las conexiones del pool y les hace ping en su lugar. [Base de datos](database.md#pool-liveness) |
...
| Familia `whereBinary()` | `Builder::where_binary` / `or_where_binary` / `where_not_binary` / `or_where_not_binary`, y `DB::table(...).where_binary(...)` | disponible | MySQL y MariaDB emiten `= binary`; Postgres y SQLite devuelven un error en lugar de una coincidencia dependiente de la intercalación. [Constructor de consultas](queries.md) |
...
| Familia `wherePivot` (incluida la forma con closure) | `where_pivot` / `where_pivot_op` / `where_pivot_in` / `where_pivot_not_in` / `where_pivot_null` / `where_pivot_not_null` / `where_pivot_between` / `where_pivot_not_between` / `where_pivot_group` más sus gemelos `or_` | diverge | Solo lectura - un filtro de pivot nunca estrecha `attach` / `detach` / `sync`, y las cargas anticipadas no lo arrastran. [Relaciones de Eloquent](eloquent-relationships.md) |
...
| API Resources | `#[derive(Resource)]` + `IntoJsonResource` + `JsonApiResponse` + fieldsets + includes | disponible | Están disponibles tanto la forma JSON:API como la forma de resource al estilo Laravel. Las rutas de `?include=` se acotan a `max_relationship_depth` (5 por defecto), igual que `JsonApiResource::$maxRelationshipDepth`. [Recursos JSON:API](eloquent-resources.md) |
...
| `refreshForUpdate()` | `model.refresh_for_update().await?` | disponible | Recarga con `SELECT ... FOR UPDATE`; el bloqueo no hace nada en SQLite. [Bloqueo de filas](eloquent.md#row-locking) |
| `inOrderOf(col, values)` | `Builder::in_order_of(col, values)` | disponible | Ordenación con `CASE WHEN` y valores vinculados; los valores no listados quedan al final. Solo en el builder tipado. [Orden](eloquent.md#ordering) |
| `orWhereKey` / `orWhereKeyNot` | `Builder::or_where_key(id)` / `Builder::or_where_key_not(id)` | disponible | Se pliegan en la cláusula anterior como una disyunción; alias `or_filter_key` / `or_filter_key_not` |
...
| Middleware del protocolo de Inertia (`Vary`, respuesta vacía, rebote de versión) | `InertiaHeadersMiddleware` + `InertiaVersionMiddleware` + `Inertia303Middleware` - tres de los middlewares que conecta `Inertia::install` (la redirección por error de validación y las páginas de error son las dos filas siguientes) | disponible | `Vary: X-Inertia` en todas las respuestas; un `200` vacío en una visita de Inertia se convierte en un `303` de vuelta; el rebote 409 vuelve a poner la sesión en flash |
...
| Páginas de error (`Inertia::render('Error', ['status' => …])` desde el handler de excepciones) | `InertiaConfig::error_page("Error")`; `InertiaErrorPageMiddleware`, conectado por `Inertia::install` cuando se nombra un componente | disponible | Una línea de configuración en vez de editar el handler de excepciones. Un `403` / `404` / `429` / `500` en una visita de Inertia o en una navegación del navegador renderiza la página nombrada con el estado original, con los props `status` / `message` / `request_id`; los clientes de API, los `422`, los rebotes con `X-Inertia-Location` y cualquier cuerpo que no sea una envoltura de error quedan intactos. Los tres starters incluyen la página y la activan. [Respuestas de Inertia](frontend-inertia-responses.md#error-pages) |
...
| `serve` | `suprnova serve` (backend + servidor de dev de Vite juntos) | disponible | [suprnova serve](cli-serve.md). Se salta el panel de Vite en un proyecto `--api` en lugar de negarse a arrancar. |
...
| `db:seed` | `cargo run --bin console db:seed` (vía el console por app) | disponible | Los sembradores se registran vía el trait `Seeder`; una ejecución dirigida imprime RUNNING / DONE con los milisegundos transcurridos |
...
| Modo de mantenimiento (`php artisan down` / `up`) | `./app down` / `./app up` - secreto de bypass con una caducidad de 12 horas comprobada en el servidor, rutas de retry/message/except personalizadas, driver `file` o `cache` | disponible | [Despliegue](deployment.md) |
