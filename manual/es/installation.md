...
- **Rust 1.94.0+** para la rama `main` actual (el workspace usa la edición 2024). La versión etiquetada v2.0.1 tiene el mismo requisito mínimo de Rust 1.94.0. Instálelo mediante [rustup](https://rustup.rs/):
  ```bash
...
fija `tag = "v2.0.1"`, y `Cargo.lock` registra el commit exacto al que
