...
- **Rust 1.94.0+** para la rama `main` actual (el workspace usa la edición 2024). La versión etiquetada v1.3.5 tiene el mismo requisito mínimo de Rust 1.94.0. Instálelo mediante [rustup](https://rustup.rs/):
  ```bash
...
fija `tag = "v1.3.5"`, y `Cargo.lock` registra el commit exacto al que
