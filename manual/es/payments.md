...
Registra un adaptador al arrancar y llama a sus operaciones soportadas a
través de los traits de pago comunes. Stripe y Paddle hidratan tablas de
copia local ligadas al cliente a partir de webhooks. El adaptador de
facturas de NOWPayments registra las notificaciones verificadas en el
registro de auditoría; las aplicaciones concilian sus propios pedidos
usando la consulta de pagos autenticada.

Los proveedores exponen el mismo contrato de traits, pero sus capacidades
difieren. Las operaciones no soportadas devuelven
`PaymentError::NotSupported`. La captura del lado del servidor y las
promociones son capacidades opcionales, que se consultan mediante
`PaymentProvider::as_payment()` y
...
suprnova-payments-stripe = { git = "https://github.com/eas4ai/suprnova.git", tag = "v1.3.7" }
```
...
```toml
# Cargo.toml
suprnova-payments-paddle = { git = "https://github.com/eas4ai/suprnova.git", tag = "v1.3.7" }
```
...
El adaptador `suprnova-payments-nowpayments` soporta facturas alojadas de
pago único e IPN verificados. Usa errores explícitos de operación no
soportada para las operaciones de cliente y de suscripción. Los eventos de
factura sin cliente se auditan sin fabricar copias locales de transacción
ligadas al cliente. Consulta la
[guía de NOWPayments](payments-nowpayments.md) para la configuración y la
conciliación.
...
los proveedores cuyas sesiones se pueden interrogar, Stripe y Paddle)
...
| `idempotency_key` | `Option<String>` | Stripe la reenvía; Paddle la rechaza si está presente |
| `metadata` | `Option<Value>` | Datos de correlación adjuntos al checkout del proveedor y a su pago o suscripción |
...
Los DTO de checkout, cargo, reembolso y cambios de suscripción exponen
un `idempotency_key: Option<String>` opcional. Fija uno en las llamadas
compatibles de Stripe:
...
Stripe reenvía las claves admitidas en el encabezado HTTP `Idempotency-Key`.
Reintenta la misma operación con la misma clave y los mismos parámetros.
Paddle no acepta claves de idempotencia suministradas por el cliente; sus
métodos de checkout y actualización de suscripciones devuelven
`NotSupported` cuando hay una clave. Guarda el ID de transacción cuando
la creación tenga éxito. Si el resultado de una creación en Paddle es
incierto, reconcilia el estado del proveedor antes de crear otra transacción.
Los metadatos de correlación no deduplican solicitudes.
