...
| Dateispeicher | `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` über OpenDAL | ausgeliefert | Dieselbe Oberfläche `put/get/delete/copy/move/exists/url`. Schutz vor Path Traversal ist eingebaut. Auf einer lokalen Disk werden `put`/`write`, der Streaming-Writer und `copy` allesamt unter `<root>/.suprnova-atomic/` zwischengelagert und in einem Schritt sichtbar gemacht, und ein bedingter Schreibvorgang wird mit `link(2)` sichtbar gemacht und bleibt so ein echtes exklusives Anlegen; `append` ist die einzige Operation, die an Ort und Stelle arbeitet. Laravels lokaler Treiber schreibt direkt auf das Ziel, wo eine unvollständige Länge beobachtbar ist und ein Absturz das Objekt an Ort und Stelle abschneidet. `Storage::register_read_through` komponiert zwei Disks zu einer Read-Through-Disk, die Treffer auf dem Fallback auf die primäre Disk hochzieht, mit `copy: false` zum Überspringen der Übernahme und mit `copy` / `rename` über die Fallback-Grenze hinweg. [Dateisystem](filesystem.md) |
...
| Inertia-Protokoll-Middleware (`Vary`, leere Response, Versions-Bounce) | `InertiaHeadersMiddleware` + `InertiaVersionMiddleware` + `Inertia303Middleware` - drei der Middlewares, die `Inertia::install` verdrahtet (der Validierungsfehler-Redirect und die Fehlerseiten sind die nächsten beiden Zeilen) | ausgeliefert | `Vary: X-Inertia` auf jeder Response; eine leere `200` bei einem Inertia-Besuch wird zu einem `303` zurück; der 409-Bounce flasht die Session erneut |
...
| Fehlerseiten (`Inertia::render('Error', ['status' => …])` aus dem Exception-Handler) | `InertiaConfig::error_page("Error")`; `InertiaErrorPageMiddleware`, von `Inertia::install` verdrahtet, sobald eine Komponente benannt ist | ausgeliefert | Eine Config-Zeile statt einer Änderung am Exception-Handler. Eine `403` / `404` / `429` / `500` bei einem Inertia-Besuch oder einer Browser-Navigation rendert die benannte Seite mit dem ursprünglichen Status und den Props `status` / `message` / `request_id`; API-Clients, `422`-Responses, `X-Inertia-Location`-Bounces und jeder Body, der kein Fehler-Envelope ist, bleiben unberührt. Die drei Starter liefern die Seite mit und aktivieren sie. [Inertia Responses](frontend-inertia-responses.md#error-pages) |
