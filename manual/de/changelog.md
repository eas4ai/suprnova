...
- **`SessionConfig` und `CookieOptions` sind jetzt `#[non_exhaustive]`.** Struct-Literale und funktionale Record-Updates im Anwendungscode müssen zu `Type::default()` gefolgt von Zuweisungen öffentlicher Felder oder Builder-Methoden wechseln.
...
- **`Prop` ist eine Struct, kein Enum.** Seine Varianten sind verschwunden; konstruieren und lesen Sie Props über Methoden:
