...
`Auth::id()` が返す、不透明な文字列識別子です - 設定済みのユーザープロバイダーがキーとする、安定したキーが何であれ、`String` としてエンドツーエンドで運ばれます。`EloquentUserProvider<User>` では、それは文字列化された主キーです。toriiバックエンドのプロバイダーでは、それはtoriiが発行したユーザーidです。セッションは `UserId` を保存し、ユーザープロバイダーのルックアップがそれを具体的なユーザー構造体へ翻訳します。この意図的な間接参照（固定された型ではなく文字列）により、ハンドラのコードを書き換えることなくユーザーバックエンドを差し替えられます。[認証](authentication.md)を参照してください。
