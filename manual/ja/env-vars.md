...
コネクションURLと、sqlxプールのチューニングです。`DATABASE_URL` は、データベースに触れるあらゆるサブコマンド（`migrate*`、`db:sync`、`db:seed`、`QUEUE_DRIVER=database` の `queue:work`、`workflow:work`、セッションのDBストア）に対して、そしてアプリにマイグレーションが登録されている場合の `serve` に対して必須です。5つの生存性のノブは、アイドルのコネクションを落とすネットワークを生き延びるための手段です - [プールの生存性](database.md#pool-liveness)を参照してください。
...
| `DB_IDLE_TIMEOUT` | 未設定（sqlxは600秒を使います） | `u64`（秒） | プールされたコネクションが、プールに閉じられるまでアイドルのまま置かれていられる時間です。`0` はアイドルの刈り取りを無効にします。 |
| `DB_MAX_LIFETIME` | 未設定（sqlxは1800秒を使います） | `u64`（秒） | プールされたコネクションが、プールに作り直されるまで生きていられる時間です。`0` は寿命による作り直しを無効にします。 |
| `DB_ACQUIRE_TIMEOUT` | 未設定（`DB_CONNECT_TIMEOUT` にフォールバックします） | `u64`（秒） | 呼び出し元が、空いたプールのコネクションをどれだけ待つかです。チェックアウトの待ちについては `DB_CONNECT_TIMEOUT` を上書きします - どちらか一方だけを設定してください、両方ではありません。ゼロは起動時に拒否されます。 |
| `DB_TEST_BEFORE_ACQUIRE` | `true` | `bool` | プールされたコネクションを手渡す前にpingを打ちます。チェックアウトごとの往復を実測したうえで `DB_PING_AFTER_IDLE` では足りないという場合を除いて、有効のままにしておいてください。 |
| `DB_PING_AFTER_IDLE` | 未設定 | `u64`（秒） | プールされたコネクションに、これだけの時間アイドルだった場合にかぎってpingを打ちます。これを設定すると `DB_TEST_BEFORE_ACQUIRE` は切れるため、直前まで使われていたコネクションは手を触れられずに手渡されます。 |
...
| `REDIS_COMMAND_RETRIES` | `0` | `u32` | 読み取り形のRedisコマンドに対する、どの読み取りにも既に付いている1回に上乗せされる追加のリトライです。キャッシュ、キュー、レートリミットの各ドライバーに適用されます。書き込みは、どの値であってもリトライしません。見積もりは秒の単位で立ててください: 切断されたコネクションに対するリトライは再接続を待つため、ドライバーのコネクトとレスポンスの予算の全体を費やします - キャッシュのドライバーでは、最大500ミリ秒の間隔で最大3回のコネクトのリトライ、そのそれぞれが2秒で上限を定められ、加えて5秒のレスポンスタイムアウトです。キューとレートリミットのドライバーでは、上限のない指数的な遅延を伴う最大6回のコネクトのリトライ、そのそれぞれが1秒で上限を定められ、加えて500ミリ秒のレスポンスタイムアウトです。`10` というクランプが縛るのは試行回数であって秒数ではありません: その設定では、1回の読み取りが12回の試行をおこないます。タイムアウトもまた一時的なものと数えられるため、停滞している間、ラップされた各読み取りはその回数までのコマンドを発行します。パースできない値は `0` にフォールバックします。 |
