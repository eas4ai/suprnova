...
Suprnovaの支払いサーフェスはプロバイダーニュートラルです。起動時にアダプターを登録し、共通の支払いトレイトを通じて、そのアダプターがサポートする操作を呼び出します。StripeとPaddleは、webhookから顧客に紐づいたミラーテーブルを構築します。NOWPaymentsのインボイスアダプターは、検証済みの通知を監査ログに記録します。アプリケーションは、認証済みの支払い照会を使って、自分自身の注文を照合します。

プロバイダーは同じトレイト契約を公開しますが、その能力は異なります。サポートされない操作は `PaymentError::NotSupported` を返します。サーバーサイドの確定とプロモーションはオプションの能力であり、`PaymentProvider::as_payment()` と `PaymentProvider::as_promotions()` を通じて問い合わせます。
...
`suprnova-payments-nowpayments` アダプターは、ホスト型の一回限りインボイスと、検証済みのIPNをサポートします。顧客およびサブスクリプションの操作については、サポートされない操作であることを示す明示的なエラーを使用します。顧客のいないインボイスのイベントは、顧客に紐づいたトランザクションミラーをでっち上げることなく監査されます。設定と照合については [NOWPaymentsガイド](payments-nowpayments.md) を参照してください。
