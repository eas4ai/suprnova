...
直接使用しても、HTTPまたはアプリケーションユーザーの所有権がMagnetarへ移るわけではありません。ホストは、レスポンスリクエスト、メール効果、アプリケーションID、レート制限ドライバー、およびセッション束縛を引き続き自身のフレームワークへ対応付けます。
