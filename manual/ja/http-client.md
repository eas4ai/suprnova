...
共有クライアントは、デフォルトでリダイレクトに従います（reqwestの上限である
10) - これは、信頼できるエンドポイントを呼んでいて、それが `http → https` で応答したり、CDNのURLを渡してきたりする場合には正しい振る舞いです。
...
`.retry()` はすべてのメソッドのトランスポート障害をリトライします。レスポンスが到着した場合は、メソッドが `POST` または `PATCH` でない限り、5xxステータスをリトライします。4xxと2xx/3xxのレスポンスは、そのまま返されます。リトライを使い切った後は、最後のレスポンス（あるいは最後のエラー）が呼び出し元へ返されます。

この区別は書き込みにとって重要です。`POST` または `PATCH` のトランスポート障害は、サーバーが書き込みをすでにコミットしていたのに、レスポンスが戻る途中で失われたことを意味する可能性がありますが、現在の契約はそれでもその障害をリトライします。これらのメソッドのために受信した5xxレスポンスは、呼び出し元が `.retry_non_idempotent(...)` を使わない限り、1回の試行の後に返されます。
...
### `.retry_when(predicate)` - ポリシーをさらに狭める
...
Laravelの `Http::` ファサードからの、3つの小さな分岐が指摘に値します。
...
**受信した5xxのリトライは、デフォルトでPOST/PATCHを拒否します。** LaravelのHTTPクライアントは、デフォルトでどのメソッドもリトライします。Suprnovaの `.retry(...)` は、`POST` と `PATCH` のトランスポート障害を引き続きリトライしますが、これらのメソッドで受信した5xxレスポンスはリトライしません。通常はアップストリームが尊重するべき等性キーを使って、書き込みを安全に再生できるようにした後でのみ、`.retry_non_idempotent(...)` を使って5xxレスポンスのリトライへオプトインしてください。

**`retry_when` は狭めるだけで、広げません。** Laravelの `retry()` の `$when` コールバックは「リトライすべきか」の決定全体を置き換えるため、フレームワークが通常は触らないステータス（たとえば404）をリトライできます。Suprnovaの `retry_when` は、`.retry(...)` / `.retry_non_idempotent(...)` がすでに行うと決めたリトライを拒否するだけです。`POST` と `PATCH` を含むすべてのメソッドのトランスポートエラーに対するリトライでは参照されますが、2xx、3xx、4xxレスポンスをリトライに変えることも、通常の `.retry()` で `POST` または `PATCH` の5xxレスポンスを対象にすることもできません。
