...
## `testing` フィーチャーと本番環境のビルド

`suprnova` は、自身のテストヘルパー（`Storage::fake()`、`TestContainer`、`TestDatabase`、`_test_install_key` のような暗号ローテーションのフック）を、`testing` という名前のCargoフィーチャーの背後に公開します。このフィーチャーはデフォルトのセットに含まれているため、依存するテストスイートはこれらを無償で手にします。
...
これらのフックは `#[doc(hidden)]` であり `_test_` で始まるプレフィックスを持つため、フィーチャーがオンでも、慣用的なアプリケーションコードから到達可能ではありません。本当に重要な安全策は `Server::from_config` です - これは、キーリングが未初期化のときだけでなく、**あらゆる**起動のたびに `APP_KEY` を検証します。事前にインストールされたテストキーが、そのチェックを回避することはできません - プロセス内で何かが事前にキーをインストールしていたかどうかにかかわらず、`APP_KEY` が欠けているか不正な形式であれば起動は早期に失敗します。

これらのヘルパーを本番環境のアーティファクトへ全く組み込みたくない場合（防御を重ねるために）、デフォルトフィーチャーをオフにして、あなたが出荷するものだけを有効にして `suprnova` に依存してください。
...
LaravelのPHPテストハーネスは、並列テストの分離をほぼ無償で手に入れます。ランタイムがリクエストごとにシングルスレッドであり、テストがファイルごとに新しいプロセスをforkするからです。Suprnovaのテストバイナリは、1つ以上のワーカースレッド上で、多数の `#[tokio::test]` を並行に実行する、1つのプロセスです。単一のグローバルコンテナでは、あるテストのフェイクが、ワーカースレッド上で重なった瞬間に次のテストのルックアップへ漏れ出してしまいます。

だからこそ `TestContainer` には両方の種類があります - よくある `current_thread` のケースのためのスレッドローカルと、`multi_thread` のためのタスクローカルです。プロセスグローバルな `ConnectionRegistry` に対する、参照カウント付きの `FAKE_GUARDS` のクリアも、同じ理由で存在します - テストごとにはできない共有状態は、少なくとも、別のテストがまだそれに頼っている間は自分自身を消し去らないと知っておかなければなりません。

マッチャーのカタログ（`expect!`）が型付けられているのは、Rustがそれを可能にするからです。Jestの `expect(x).toBeSome()` は、`x` が `Option` かどうかを実行時にしか知りませんが、Suprnovaの `Expect<T>` はコンパイル時にそれを知っているため、間違ったマッチャーはビルドエラーであり、不安定なテストではありません。
