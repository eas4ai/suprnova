...
バスは、Suprnovaの**同期的な**コマンドディスパッチャーです。型付きの `Command`（`{ input, Output type }`）を定義し、起動時にそれ用の `Handler` を登録すれば、プロセス内のどのコードからでも `Bus::dispatch(cmd).await` を呼び出し、ハンドラの型付きの結果を運ぶ `Dispatched<T>` を受け取れます。
