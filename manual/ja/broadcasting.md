...
| `max_missed_pings` | 2 | 1回の見逃したPongで即座に閉じるべきチャットには `1` を設定する。不安定なモバイルネットワークには `3+` を設定する。no-pong時のクローズを無効化するには `usize::MAX` を設定する。 |
...
デフォルトの `InMemoryBroadcastHub` は、現在のプロセス上の購読者へしかファンアウトしません。マルチレプリカのデプロイでは、`broadcasting-fanout` というCargoのフィーチャーを有効にし、`SeaStreamerBroadcastHub` に差し替えてください:
...
コンストラクタは2つの引数を取ります: ストリーマーのURI（スキームによって実行時にバックエンドを選択します）と、ストリームキー（クラスタ内のすべてのプロセスで共有されるトピック名）です。すべてのレプリカで同じストリームキーを使ってください - そうしないと、互いのイベントが見えません。

`new_with_presence_ttl(uri, key, ttl)` は、デフォルトの60秒のプレゼンスTTLを上書きします - クラッシュからの復旧経路を素早く動かす必要のあるテストで役に立ちます。`new_loopback(uri, key)` は、単一プロセスの統合テスト向けにstdioのループバックを有効にします。重複ガードによって、各アプリイベントはローカルでちょうど一度だけ配信されることが保証されます。

### バックエンド

バックエンドは、URIのスキームから実行時に選択されます:

| URIスキーム | バックエンド | 本番利用可 | 備考 |
|------------|---------|------------------|-------|
| `redis://`, `rediss://` | Redis Streams | **はい** | デフォルトの推奨。`rediss://` はTLSを使います。デフォルトビルドで有効です。 |
| `kafka://`, `kafka+ssl://` | Kafka | **はい** | `sea-streamer` のフィーチャーセット（`framework/Cargo.toml`）に `kafka` が必要です。 |
| `stdio://` | stdin/stdoutのパイプ | いいえ - テスト専用 | 単一プロセスのループバック。 |
| `file://` | ローカルファイル | いいえ - 単一ホスト | `sea-streamer` のフィーチャーセットに `file` が必要です。 |

デフォルトのSuprnovaビルドは、`stdio` + `redis` + `socket` を有効にします。Kafkaまたはfileを有効にするには、`framework/Cargo.toml` を編集して、該当する `sea-streamer` のフィーチャーを追加してください。

### アーキテクチャ

各 `publish(envelope)` は、2つのことを並行して行います:

1. **ローカルのファンアウト** - 内側の `InMemoryBroadcastHub` が、このプロセス上の購読者へただちに配信します。ローカルの購読者がネットワークを待つことは決してありません。
2. **ストリームへの書き込み** - 同じエンベロープがシリアライズされ、sea-streamerのストリームへプッシュされます。そのため、他のすべてのプロセスのコンシューマーポンプがそれを拾い上げ、ローカルで配信します。

重複配信のガードが、各アプリデータのイベントを二度見ることを防ぎます: hubのインスタンスはランダムなUUIDを持ち、それが生成するすべてのエンベロープはそのUUIDを運び、コンシューマーポンプは、インスタンスIDがローカルのhub自身のものと一致する受信エンベロープをスキップします。プレゼンスのメタチャネルのメッセージは例外です - 読み取り経路を統合するために、各hubはクロスプロセスのビューの中に自分自身のイベントを必要とするからです。

バックエンドのディスパッチは、トレイトオブジェクトではなくenumベースです: hubは、sea-streamerのソケットアダプターの具体的な `SeaProducer` / `SeaConsumer` を保持しており、これはコンパイルされたすべてのバックエンドを網羅するenumです。publishの呼び出し箇所に `dyn` のオーバーヘッドはありません。

### クロスプロセスのプレゼンス

`SeaStreamerBroadcastHub` は、プレゼンスの状態をプロセスをまたいで自動的にレプリケートします。各インスタンスは、構築時にUUIDの `instance_id` を持ちます。`track_member` / `untrack_member` は、予約された `__presence__` メタチャネルへ `PresenceEvent` を発行します。すべてのプロセスは、自身のコンシューマータスクによって更新される `cross_process_view` を維持します。`list_members` は、統合されたビュー（ローカルとリモートを一様に）を返します。

生存確認: 各プロセスは、ハートビートとして `ttl / 6`（デフォルトの60秒TTLでは10秒）ごとに自身のメンバーを再発行します。古くなったエントリ - `last_seen` がTTLを超えたメンバー - は、`ttl / 2` ごとに刈り取られます。これは、`MemberRemoved` を発行するに至らなかったプロセスのクラッシュに対処します。
