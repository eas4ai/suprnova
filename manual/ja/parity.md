...
| ファイルストレージ | OpenDAL の上の `Storage::disk("local"\|"s3"\|"azblob"\|"gcs"\|"memory")` | 実装済み | 同じ `put/get/delete/copy/move/exists/url` の表面です。パストラバーサル保護が組み込まれています。ローカルディスクでは、`put`/`write`、ストリーミングのライター、そして `copy` のいずれもが、まず `<root>/.suprnova-atomic/` の下に書き出されてから、1ステップで公開されます。条件付きの書き込みは `link(2)` で公開されるため、本物の排他的な作成であり続けます。`append` だけが、その場で行われる唯一の操作です。Laravel のローカルドライバーは行き先へ直接書き込むため、途中までの長さが観測でき、クラッシュは生きているオブジェクトを切り詰めてしまいます。`Storage::register_read_through` は、2つのディスクを、フォールバックでのヒットをプライマリへ昇格させるリードスルーディスクへ合成します。昇格をスキップする `copy: false` と、フォールバックをまたぐ `copy` / `rename` を備えます。[ファイルシステム](filesystem.md) |
...
| Inertiaプロトコルのミドルウェア（`Vary`、空のレスポンス、バージョンの跳ね返し） | `InertiaHeadersMiddleware` + `InertiaVersionMiddleware` + `Inertia303Middleware` - `Inertia::install` が配線するミドルウェアのうち3つ（検証エラーリダイレクトとエラーページは次の2行） | 実装済み | すべてのレスポンスに `Vary: X-Inertia`。Inertiaの訪問での空の `200` は `303` の戻しになります。409の跳ね返しはセッションを再フラッシュします |
...
| エラーページ（例外ハンドラからの `Inertia::render('Error', ['status' => …])`） | `InertiaConfig::error_page("Error")`。コンポーネントが名指しされているとき `Inertia::install` が配線する `InertiaErrorPageMiddleware` | 実装済み | 例外ハンドラに手を入れる代わりに、設定1行で済みます。Inertiaの訪問やブラウザのナビゲーションでの `403` / `404` / `429` / `500` は、名指しされたページを元のステータスのまま描画し、`status` / `message` / `request_id` のプロップを伴います。APIクライアント、`422`、`X-Inertia-Location` の跳ね返し、そしてエラーのエンベロープではないボディは、手を触れられません。3つのスターターは、このページを同梱して有効にしています。[Inertia レスポンス](frontend-inertia-responses.md#error-pages) |
