...
suprnovaは、タスクがいつ実行されるべきかを定義するための、フルーエントなAPIを提供します。
...
|--------|-------------|
| `.every_minute()` | 毎分実行する |
| `.every_two_minutes()` | 2分ごとに実行する |
| `.every_five_minutes()` | 5分ごとに実行する |
| `.every_ten_minutes()` | 10分ごとに実行する |
| `.every_fifteen_minutes()` | 15分ごとに実行する |
| `.every_thirty_minutes()` | 30分ごとに実行する |
| `.hourly()` | 毎時0分に実行する |
| `.hourly_at(30)` | 毎時30分に実行する |
| `.every_two_hours()` / `.every_three_hours()` / `.every_four_hours()` / `.every_six_hours()` | N時間ごとに、正時に実行する |
| `.daily()` | 毎日、深夜0時に実行する |
| `.daily_at("03:00")` | 毎日午前3時に実行する |
| `.twice_daily(1, 13)` | 1日2回実行する（例: 午前1時と午後1時） |
| `.weekly()` | 毎週日曜の深夜0時に実行する |
| `.monthly()` | 毎月1日の深夜0時に実行する |
| `.monthly_on(15)` | 毎月、指定した日に実行する |
| `.quarterly()` | 1月/4月/7月/10月の1日の深夜0時に実行する |
| `.yearly()` | 1月1日の深夜0時に実行する |
...
特定の時刻を設定するには、どのスケジュールにも `.at()` をチェーンしてください。

```rust
.daily().at("14:30")           // 毎日午後2時30分に
.weekly().at("09:00")          // 毎週午前9時に
.mondays().at("08:00")         // 毎週月曜午前8時に
.monthly().at("00:00")         // 月初の深夜0時に
```

### カスタムCron式

完全な制御が必要な場合は、cron構文を使ってください。

```rust
// 標準的なcronの形式: 分 時 日 月 曜日
.cron("0 */2 * * *")    // 2時間ごと
.cron("30 4 * * 1-5")   // 平日の午前4時30分
.cron("0 0 1,15 * *")   // 毎月1日と15日
```

式が不正な形式である場合（フィールド数の誤り、パース不能なstep/range/list）、`.cron(...)` は**パニックします**。式が実行時に与えられる場合（設定、ユーザー入力）で、パースエラーを伝播させたいのであれば、`.try_cron(expr)` を使ってください。

```rust
schedule.add(
    schedule.task(MyTask::new())
        .try_cron(env_expr)?   // 不正な式に対して Err(String) を返す
        .name("from-config")
);
```

同じ `panic` / `try_*` のペアは、数値範囲を取るすべてのビルダーメソッドに存在します: `try_hourly_at`、`try_daily_at`、`try_twice_daily`、`try_monthly_on` です。失敗しないほうのバリアントは、範囲外の数値（例えば `daily_at("25:00")` や `monthly_on(40)`）でパニックします。失敗しうる兄弟のほうは `Err(String)` を返します。
...
このプロセス内のゲートは、`without_overlapping` とは無関係に、**常に有効です**。これはプロセスをまたぎません（各プロセスは、タスクごとの状態を自分自身で持ちます）。プロセスをまたぐ同一分の協調が必要な場合は、`without_overlapping` を重ねてください
+ 設定済みのCacheバックエンドを使ってください - 両方を合わせれば、両方の方向をカバーします。
...
実行予定のタスクをすべて一度だけ実行します（典型的には、cronから毎分呼び出されます）。
