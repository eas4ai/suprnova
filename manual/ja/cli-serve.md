...
7. バックエンドのために、サーバーが実際にそこからビルドされるパス - `src/`、`cmd/`、`Cargo.toml`、`Cargo.lock`、`.env`、そして `lang/` - へ `-w` でスコープを絞った `cargo watch` を起動する。`cmd/` は、フルスタックのスキャフォルドがサーバーバイナリの `main.rs` を置く場所です。`--api` のスキャフォルドはそれを `src/` に置き、`cmd/` を持ちません。各パスは、それが存在するときにだけ渡されます。cargo-watchは、存在しない `-w` のパスの上では起動を拒むからです - まだ一度もビルドされていないプロジェクトは `Cargo.lock` を持たず、それは次の `serve` で拾われます。

   `--no-vcs-ignores` はそれらと一緒に渡されます。cargo-watchは、自分自身のプロジェクト走査に対してだけでなく、明示的に名指しされた `-w` のルートに対してもあなたの `.gitignore` を適用し、スキャフォルドは `.env` をgitignoreします - つまり、そのフラグがなければ `-w .env` は何ひとつ監視しません。このフラグが、バックエンドを再起動させるものを広げてしまうことはあり得ません。`-w` がその範囲を上の6つのパスへ既に絞っており、そのなかでgitignoreされているのは `.env` と（`--api` では）`Cargo.lock` だけで、どちらも意図して監視しているからです。`target/`、`node_modules`、その他はいずれにせよ、監視対象のどのルートの外側にも位置しています。

   スキャフォルドされたフルスタックのプロジェクトでは、完全な起動コマンドは `cargo watch --no-vcs-ignores -w src -w cmd -w Cargo.toml -w Cargo.lock -w .env -w lang -x 'run --bin <package-name>'` です。フロントエンドの編集と、生成された `frontend/src/types/*.ts` はそのスコープの外にあるため、バックエンドを再起動させることは決してありません。
...
10. `src/` に対するファイルウォッチャーを起動し、`.rs` ファイルが変更されるたびに、保存のバーストが500ms静かになった時点で型ジェネレーターを再実行する。数に入るのは本物の変更 - 作成、書き込み、削除 - だけです。読み取りは数に入りません。これが重要なのは、ジェネレーターが実行のたびに、ウォッチャーが監視しているのと同じツリーの配下にあるすべての `.rs` ファイルを読むからです。ステップ4の起動時の型生成と同じく、プロジェクトにフロントエンドがない場合はスキップされる。デバウンスはトレイリングエッジ方式であるため、バースト - `cargo fmt`、複数ファイルをまたぐformat-on-save、ブランチの切り替え - は、最初のファイルで発火して残りを見逃すのではなく、最後の書き込みの*後に*走る、ちょうど1回の再生成へまとめられます。再生成は、出力されるTypeScriptが既にそこにあるものと異なるときにだけファイルを書き、ウォッチャーは自分が書いたものだけを報告します: どのプロップの形状も変えない編集は、何も出力せず、`types_regenerated` イベントも発行しません。保存の後の沈黙は、あなたの編集が生成された型を変えなかったということです。
...
**バックエンド。** `cargo watch` がそのループで、サーバーがそこからビルドされるパスへスコープを絞られています。`src/` または `cmd/` の配下での変更、`Cargo.toml`、`Cargo.lock`、`.env` への変更、あるいは `lang/` の配下での変更ごとに、再ビルドして再起動します - `.env` は起動時に `Config::init` が一度読み、Fluentのカタログはブートストラップで一度読まれるため、どちらへの変更も再起動のときにだけ効きます。`.env` は `--no-vcs-ignores` を通して監視されており、これがなければあなたの `.gitignore` がそれをウォッチャーから隠してしまいます。コンポーネントを保存すること、あるいは `frontend/src/types/inertia-props.ts` を再生成することは、そのスコープの外にあるため、バックエンドは動いたままになります。重いクレートに触れた後のコールドリビルドは数秒かかることがあります。単一ファイル内の差分ビルドは、通常1秒未満です。

**フロントエンド。** ViteのHMRは、コンポーネントの状態を保ったまま、フルリロードなしでコンポーネントの変更をその場に注入します。Tailwindのクラスは、Tailwind v4のウォッチャー経由でライブに更新されます。

**TypeScriptの型。** `.rs` ファイルが変更されるたびに、型ウォッチャーはジェネレーターを再実行します。新しい `#[derive(InertiaProps)]` 構造体が現れる（あるいは既存のものが形状を変える）と、再生成された `frontend/src/types/inertia-props.ts` は、それらをインポートしているコンポーネントに対してViteのHMRを引き起こします。出力されるTypeScriptが、既にディスク上にあるものとバイト単位で同一である場合、ファイルは触れられないままとなり、ウォッチャーは何も言いません。つまり、何も変えなかった再生成は、下流の何かが反応しなければならない変更ではありません - Viteにとっても、バックエンドのウォッチャーにとっても、`--json` を読んでいる何かにとってもです。
...
| `types_regenerated` | `ts`、`artifact`（`"inertia_props"` または `"lang_keys"`）、`count` | `.rs`/`.ftl` の変更の後で、ファイルウォッチャーがTypeScriptアーティファクトを書き直した。発火するのは、生成されたファイルが実際に変わったときだけです: 出力されるTypeScriptがバイト単位で同一のままになる `.rs` の編集は、何も書かず、何も発行しないため、イベントがあるということは、ディスク上のファイルが今は違うということです。`count` は、書き直されたファイルの中の構造体（あるいはメッセージID）の数であって、変わった数ではありません。 |
