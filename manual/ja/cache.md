...
- サブ秒の `Duration` は、`EX` の下では `0 seconds` に切り詰められてしまいます。Redisはこれを拒否する（`SET … EX 0`）か、さらに悪いことに「キーを削除する」（`EXPIRE key 0`）と解釈します。
- `Duration::ZERO` は、呼び出しの前に1ミリ秒へクランプされるため、どちらの拒否経路もユーザーコードから到達することはありません。

### 一時的なコマンドのリトライ

かつては、ソケットが切断されると、その時点で実行中だった `Cache::get` が何であれ失敗していました。Redisのコネクションマネージャーは自力で再接続しますが、死んだソケットに当たったコマンドは、依然としてそのエラーをあなたに返します。

読み取り形のコマンドは、いまでは一度だけリトライします: `GET`、`EXISTS`、そして `Cache::flush` と `Cache::flush_tags` の背後にある `SCAN` / `SSCAN` のページです。キューのドライバーの `XLEN`、`ZCARD`、`XPENDING` の読み取りと、レートリミッターの `Retry-After` の計算も、同じようにリトライします。組み込みの1回に上乗せしてさらにリトライを重ねるには、`REDIS_COMMAND_RETRIES` を設定してください。

リトライの見積もりは、その前に置かれる50ミリ秒の休止ではなく、秒の単位で立ててください。コネクションがいったん切断されると、次の試行は、何かを送信できるようになる前に、代わりのコネクションを待ちます。そのため、ドライバーのコネクトの予算の全体を、続いてそのレスポンスタイムアウトを支払うことになります:

- キャッシュのドライバーは、最大500ミリ秒の間隔で最大3回のコネクトのリトライを許し、そのそれぞれは2秒のコネクトタイムアウトで上限が定められ、レスポンスタイムアウトは5秒です。
- キューとレートリミットのドライバーは、redis-rsのデフォルトを取ります: 100ミリ秒から始まる上限のない指数的な遅延を伴う、最大6回のコネクトのリトライで、そのそれぞれは1秒のコネクトタイムアウトで上限が定められ、レスポンスタイムアウトは500ミリ秒です。

`REDIS_COMMAND_RETRIES` は10でクランプされますが、そのクランプが縛るのは試行回数であって、秒数ではありません: 最大値では、1回の読み取りが12回の試行をおこない、Redisがダウンしていれば、これは1回の呼び出しで数十秒から数分になります。タイムアウトしたコマンドも、切断されたコマンドと同じく一時的なものと数えられます。そのため、単に遅いだけのRedisであっても、ラップされたすべての読み取りが、1回ではなくその回数だけコマンドを発行することになります。この設定を引き上げるのは、呼び出し元が待つ余裕のある場所に限ってください。

書き込みは、どの設定であっても決してリトライしません。一時的なエラーが意味するのは、コネクションが失敗したということであって、サーバーがそのコマンドを拒んだということではありません - サーバーは既にそれを実行してしまっているかもしれません - そのため、`SET`、`INCR`、ロックの獲得、レートリミットのヒット、あるいはキューのポップをリトライすることは、二重に実行されるリスクを冒します。これらのコマンドは失敗をあなたに表面化させ、リトライするかどうかの判断は、事情を把握したうえでのあなたのものになります。

### Suprnovaが異なる設計を選んだ理由

Laravelの `command_retries` 設定は、すべてのRedisコマンドについてリトライの予算を引き上げます。というのも、Laravelの `command()` メソッドは、すべてのコマンドが通る単一の地点であり、自分がどのコマンドを実行しているかを知っていて、60エントリの読み取り専用の許可リストを参照するからです。Suprnovaのドライバーは型付きのコマンドを直接呼び出すため、この許可リストは呼び出し箇所ごとの判断になり、`REDIS_COMMAND_RETRIES` が深められるのは、繰り返しても既に安全なコマンドのリトライだけです。キューのポップをリトライさせる設定は存在しません。
## テスト
...
フレームワーク自身のRedisのテストは `#[ignore]` されているため、`cargo test` がサーバーを必要とすることはありません。実行するには `-- --ignored` を付け、インスタンスを指し示してください:

- `cache_redis_integration` は `CACHE_REDIS_TEST_URL` を読み、なければ `REDIS_URL` へ、さらになければ `redis://127.0.0.1:6379` へフォールバックします。各テストは自身を一意なキーのプレフィックスにスコープするため、共有された開発用のRedisに対しても安全です。
- `cache_redis_retry` は、一時的なコマンドのリトライを対象とし、フォールバックなしで `CACHE_REDIS_TEST_URL` を明示的に要求します。このテストは `CLIENT KILL TYPE normal` を発行し、これはそのインスタンス上の他のすべてのクライアントを切断するため、使い捨てのサーバーを与えなければなりません。この変数が設定されていない場合は、スキップ行を出力し、接続せずにパスします。
## パターン
