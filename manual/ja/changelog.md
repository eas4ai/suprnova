...
- **Live がコンポーネントライブラリの基盤を同梱するようになりました。** ドキュメントが `with_suprnova_ui()` でオプトインすると、ベースレイヤー付きのトークンスタイルシートがランタイムアーティファクト `ui-styles` として届きます。すべてのルールはカスケードレイヤー `suprnova-ui` にあり、すべての視覚的な値はライトとダークの値を持つ `--sn-` トークンなので、スキンを外しても何も壊れません。`live:add` はコンポーネントをマニフェストから `templates/suprnova-ui/` 配下のひとつのディレクトリとしてインストールし、編集済みのファイルを保持し、サードパーティのマニフェストを独自のルート配下に受け入れます。フレームワークは取り込まれたスタイルシートとスクリプトを `/suprnova-ui/<component>/<file>` で配信します。フォームファミリーはチェッカーが展開するようになった Askama マクロとして提供され、ライブラリのビューは他のビューと同じように `live:check` を通過します。`suprnova.` 名前空間はライブラリに予約され、レジストリは他のクレートからのこの名前空間を拒否します。
- **Live のコンポーネントライブラリにオーバーレイの一族が加わりました。** ツールチップ、コラプシブルとアコーディオン、ポップオーバー、単一階層のドロップダウンメニュー、ダイアログ、シート、ドロワーが、フォームの一族と並んで `live:add` でインストールされます。それぞれはスクリプトより前にネイティブのプリミティブ（`details`、`popover` 属性、`dialog`）で開閉状態を保持し、開閉のために Live リクエストを発行せず、閉じるときにフォーカスをトリガーへ返し、`live:preserve.self` を伴う安定したキーのもとでモーフをまたいで開閉状態を保ちます。
- **Live コンポーネントライブラリにフィードバックファミリーとナビゲーションファミリーが加わりました。** アラート、スケルトン、スピナー、プログレス、空状態、トースト領域とフラッシュ領域、ヘッダーバー、フッター、サイドバー、パンくずリスト、タブ、ページネーション、さらに読み込むが `live:add` でフォームファミリーとオーバーレイファミリーの隣にインストールされます。フィードバックはサーバーまたはランタイムがすでに持つ状態を表示します。アラートのロールはバリアントに従い色以外の手がかりを持ち、読み込み表示は `live:loading` で結び付けられランタイム自身のタイミングで現れ、プログレスはラベルと読み上げ付きのネイティブ要素で、空状態の理由はサーバー状態であり、トーストはフォーカスを奪わず一度だけ通知し、重大なエラーはアラートとしても描画されます。ナビゲーションはルートの意味論を保ちます。実 URL を持つアンカー、サーバー由来の `aria-current`、タブとページネーションの明示的なローカルまたはルートモード、履歴エントリを作らず `history.replaceState` で反映される Live ページネーション、そしてボタンからのキー付き追記としてのさらに読み込むです。ビューフィルター `live_key` もこれらと共に入ります。`{% for %}` ループ内の `live:key` はこのフィルターを通り、チェッカーがリテラルのキーに課すキー規則を描画時に強制します。
- **Live コンポーネントライブラリにデータ表示ファミリーが加わり、組み込みセットが完結しました。** セパレーター、スクロール領域、アスペクト画像、カード、バッジ、アバターとアバターグループ、リストグループ、説明リスト、統計カードが `live:add` で先行ファミリーと並んでインストールされます。それぞれが文書の順序とネイティブの意味論を保ち、すべての状態がテキストを持ちます。チャートはサーバーで `charts-rs` により描画されます。`suprnova::live::charts` の `render_chart` が範囲を限定した型付きの系列から棒または折れ線のマークを信頼済み SVG として描き、マクロがその横に要約とデータ表を置くため、チャートのスクリプトはブラウザーに届きません。データテーブルは表ごとに 1 つのアイランドを持つネイティブの表で、並べ替え、フィルター、ページは各アクションの後に共有可能な URL へ反映される `#[url]` フィールドです。
- **Live コンポーネントライブラリに最後のファミリーである live-native ファミリーが加わりました。**
  アップロードウィジェットは同梱のアップロードプロトコルの状態をランタイムの進捗ルートからレンダリングし、確定アクションの前には何も永続化されたと主張しません。ライブフィードと通知ベルはランタイムのストリーム状態アナウンスを持ち、劣化、再接続中、または閉じたストリームはそのように述べます。アカウントメニューは CSRF で保護されたサインアウトフォームと、RenderCache 配下の文書化されたスティッチスロットを持つ details 開閉要素です。カスタム要素層 (OTP 入力、日付ピッカー、コンボボックス) は決して置き換えないネイティブコントロールを強化します。各 `sn-` 要素は自身のベンダー化ファイルが定義する light DOM の `HTMLElement` で、フォーム値を保持しないため、スクリプトをブロックしてもフォームは同じ値を送信します。コンボボックスは古いクエリに対してレンダリングされたリストボックスを拒否します。
- **セッションブロッキングが同じセッションを持つリクエストを直列化します。** `SESSION_BLOCK=true` または `SessionConfig::block(SessionBlock::default())` により、セッションミドルウェアは読み込みから書き込みまでセッションのキャッシュロックを保持し、ルートやグループの `block_session` はそれらのルートだけで有効にします。保持時間と待機時間の両方の上限はあなたが決め、上限を超えて待ったリクエストは `Retry-After` 付きの `503` で応答します。これがないと、1 つのセッション上の 2 つの同時リクエストは最後に書いた方が勝つ形で書き戻され、リダイレクトが設定したフラッシュが先に始まったリクエストに上書きされて失われることがありました。

### 変更

- **Live のエンドポイントからバージョンセグメントがなくなりました。** `/__live/v1/action`、`/__live/v1/upload`、`/__live/v1/assets/*`、そして `/__live/v1/async/*` の一族は、`/v1` を除いた同じパスに移りました: `/__live/action`、`/__live/upload`、`/__live/assets/*`、`/__live/async/*`。Suprnova のバージョンは git タグであり、ブラウザーランタイムはフレームワークと足並みをそろえて出荷されるので、URL 空間の中の第二のバージョンは、決して使われない進化の道を約束するだけでした。アプリケーションには影響ありません。これらのルートはフレームワークが登録し、URL はすべてランタイムが組み立てます。`/__live/` パスを手書きで参照することは、もともとサポートされていませんでした。
- **Live がサポートするブラウザーの基準は Chrome と Edge 114、Firefox 128、Safari 17 になりました。** 下限は Chrome と Edge 111、Safari 16.4 から、コンポーネントライブラリのオーバーレイが開閉状態を保持するのに使うネイティブの `popover` 属性を最初に搭載したバージョンへ引き上げられました。互換性マトリクスの最小スロットもそれに合わせて動きます。Firefox は 128 のままです。

### 修正

- **テンプレートの `live:key` が Live ランタイムに届くようになりました。** チェッカーは `live:key` を検証し、マニュアルもそれを挙げていましたが、ブラウザーランタイムのモーフ同一性、モーフ制御、保持スコープはエンジンの `data-suprnova-live-key` しか読まなかったため、チェッカーの要求どおりに書かれたキー付きコントロールは効果がなく、すべてのライブラリコンポーネントがキーを 2 回書いていました。ランタイムは今後 `live:key` を読み、エンジンが描画するルートにはエンジンの表記を残し、両方を異なる値で持つ要素を拒否し、コンポーネントは `live:key` だけを書きます。フォームギャラリーの保存フォームにも欠けていた `.prevent` 修飾子が加わり、Live の送信がフォーム自身のクエリからページを再読み込みしなくなりました。
- **空の数値入力や未選択のセレクトを含む Live フォームが送信されるようになりました。** ランタイムはそのようなコントロールの null 値を自分自身との不一致として読み、送信全体を拒否していたため、アクションは実行されず、ブラウザーがフォームをネイティブに送信していました。検索入力のモデルバインディングもランタイムが受け付けないデバウンスを使っていましたが、現在は 250 ms を使います。
- **`live:check` がビューの描画するすべての要素を検査するようになりました。** `caller()` を差し込むマクロを空の call ブロックで呼び出すビュー（バリデーションサマリーの呼び出し方）は、その呼び出し以降の内容をチェッカーに渡しておらず、ビューの残りが未検査のままコンポーネントが証明済みになっていました。空の呼び出しは空の内容になり、何も描画しないビューは検査に失敗します。この修正により、ドッグフードのフォームギャラリーが隠していたエラーが明らかになりました。
- **宣言するモデルのデバウンスは 100、250、500 ミリ秒のいずれかになりました。** `#[model(debounce = N)]` は 1 から 60000 ms を受け付けていましたが、チェッカーとブラウザーランタイムが共有するディレクティブ文法は 3 つの時間しか列挙しないため、それ以外の値はテンプレートから決して束縛できませんでした。現在はコンパイルに失敗します。`live:error` はバリデーションサマリーが指定するようにアクションもターゲットとして受け付け、ランタイムの解決方法と一致します。
- **7 つを超えるモデルフィールドを持つ Live フォームが送信されるようになりました。** ブラウザーランタイムは、フレームワークのサーバーがそれぞれ 128 まで受け付けるにもかかわらず、8 つを超える操作やモデル提案を含むリクエストと、16 を超えるバリデーション項目や 8 つを超えるイベントを含むレスポンスを拒否していました。拒否された送信はブラウザーの外に出ず、ネットワーク障害として報告されていました。ブラウザーはサーバーの件数を受け付けるようになり、`live:check` は 127 を超えるモデルフィールドを持つ `live:submit` フォームを拒否し、上限で拒否されたリクエストはアクションのエラーフィードバックを表示するリソース上限として報告されます。
- **Live のスコープごとの上限が受け入れたすべての発行が、接続できるサブスクリプションで応答するようになりました。** 同じスコープの同時発行が同じミリ秒に行われると同一のディスクリプタが生成されますが、ホストの資格情報ストアはひとつのディスクリプタに対して最後に生成されたシークレットしか保持していなかったため、先の要求は発行が行う接続の段階で 403 `async_authority_invalid` を返していました。また、およそ五百件に一件の発行は 503 `async_unavailable` を返していました。エンベロープコンテキストの構築中に公開するクレームが、同時に行われるすべての発行とひとつのスロットを共有していたためです。ディスクリプタは今では未消費のシークレットをそれぞれが消費または失効するまで保持し、構築中のクレームはサブスクリプション ID ごとに管理されます。
- **コンボボックスが応答し続け、サーバーが返したものを表示するようになりました。** どの選択肢にも一致しないテキストでページが固まっていました。要素のオブザーバーが、何も変わらない場合でも自身の描画が書き換える属性を監視していたためです。要素はまた、サーバーの選択肢を部分文字列で絞り込んでいたため、アクセントを区別しない一致やコードでの一致など、サーバー検索が別の方法で一致させた結果を隠していました。現在は変更された部分だけを書き込み、リストボックスが入力の現在のテキストに応答している間はすべての選択肢を表示し、古いテキストへの応答は非表示のままにし、入力されたテキストによる絞り込みは `remote=false` で選択する固定リストの場合にだけ行います。
- **ライブラリのカスタム要素が、モーフによってどのように移動されても一度だけバインドするようになりました。** コンボボックス、OTP 入力、日付ピッカー、パスワード入力は接続のたびにリスナーをバインドし、ダイアログ、シート、ドロワーはモーフが削除する属性でガードしていたため、移動した要素は 1 回のクリックに 2 回応答し、パスワードの表示切替はすぐに元に戻っていました。現在、各要素は接続ごとにバインドし、ドキュメントから離れるときにリスナーとオブザーバーを解放し、アトミックな移動の間はそれらを保持します。
- **ポインターがトーストのどこかにある間、またはフォーカスがその内側にある間、トーストが保持されるようになりました。** 領域はポインターがいずれかの子要素から離れるとタイマーを再開していたため、トーストのテキストから余白へポインターを移すとポインターの下でタイムアウトし、閉じるボタンにフォーカスがある間にトーストが消えることもありました。
- **入れ子のタブが自身のタブに作用するようになりました。** ローカルのタブインスタンスが配下のすべてのタブを選択し、入れ子のインスタンスのイベントも処理していたため、内側のタブをクリックすると外側のパネルが隠れていました。
- **ポインターが吹き出しへ移動する間、ツールチップの吹き出しが開いたままになるようになりました。** 吹き出しがポインターを無視していたため、ポインターがトリガーを離れると隠れてしまい、そのテキストを読むことも選択することもできませんでした。
- **セレクトのドロップダウンインジケーターがテキストの色に従うようになりました。** インジケーターはデータ URI の SVG で、その `currentColor` はドキュメントの色を継承しないため、ダークスキームの面の上に黒で描かれていました。
- **フォームコントロールがアイランドの値を表示するようになりました。** どのフォームマクロも値を受け取らなかったため、1 でマウントされた数値入力は空で描画され、何も変更しない送信は空の値を提案していました。各値コントロールは `value=`、`checked=`、`selected=` を受け取るようになり、ラジオグループとチェックボックスグループの入力は値をキーとするため、ユーザーがまだ送信していない選択は再描画を生き延び、`authority=` はユーザーの入力を置き換えなければならない描画を示します。
- **チェックボックスグループがチェックされた値のリストを提案するようになりました。** ランタイムはすべてのチェックボックスを真偽値として読んでいたため、ボックスの状態が揃わないグループは送信できませんでした。サイズを問わないチェックボックスグループも、複数のチェックボックスが結び付く任意のフィールドも、チェックされた値をドキュメント順で提案するようになりました。単独のチェックボックスは真偽値のままです。
- **フィールドがデコードできないモデル提案は、そのフィールドのバリデーションエラーになりました。** `u64` フィールドへの null やリストフィールドへの真偽値のような提案は黙って破棄され、アクションは実行され、レスポンスにはバリデーションが含まれていませんでした。現在、フィールドは `live:error` を通じてエラーを報告して値を保ち、アクションは実行されません。
- **チェッカー、`live_key` フィルター、ランタイムが同じ 1 つのキーのアルファベットを受け付けるようになりました。** チェッカーとフィルターは `_`、`-`、`.`、`:` で始まるキーを受け付けていましたが、ランタイムはそれを拒否するため、アイランドの最初のモーフが失敗していました。現在、キーはどこでも ASCII の英字または数字で始まり、`live:check` はアイランド内の要素 ID にランタイムが検査するルールを適用し、無効な ID (`invalid_element_id`) と重複した ID (`duplicate_element_id`) を、ループ内のリテラル ID も含めて拒否します。
- **`live_key_digest` が任意の値をキーにするようになりました。** `live_key` はキーのアルファベット外の値に対してアイランドの描画を失敗させるため、メールアドレスをキーにした行があると、すべての閲覧者に対してアイランドが失敗していました。新しいフィルターは任意の値を安定したキーに変換し、同じ値は常に同じキーになります。
- **`live:check` がループや match の束縛をその束縛として検査するようになりました。** マクロ本体の中で、`for`、`match` のアーム、`if let` が束縛した名前は同じ名前のマクロパラメーターとして検査されていたため、リテラルの引数が、ループ自身の値が描画するディレクティブを証明していました。
- **`render_chart` が 1e9 を超える値に対してエラーを返すようになりました。** レンダラーの軸の計算はおよそ 1e12 からオーバーフローしてパニックしていました。現在、絶対値が 1e9 を超える値は入力エラーです。
- **ベンダリングしたコンポーネントを読み込めないアプリケーションが起動を拒否するようになりました。** `try_live_ui_assets()` はリクエストのたびにアプリケーションのベースパス配下の `templates/suprnova-ui/` を読み込むため、バイナリだけを含むコンテナイメージからなど別の場所で起動したアプリケーションは、すべてのコンポーネントのスタイルシートとスクリプトに 404 を返しながら、問題なく起動していました。現在はルートのインストールが失敗し、そのディレクトリを示します。
- **`live:add` が一度も編集していないファイルを置き換えるようになりました。** バイトだけを比較していたため、ライブラリの更新後もインストール済みのすべてのファイルを保持し、ローカルで編集されたと報告していました。現在は各ファイルのダイジェストをコンポーネントの隣に記録し、バイトがまだ記録と一致するファイルを置き換えます。編集したファイルや、どの記録も裏付けないファイルは保持され、報告されます。
- **`live:add --manifest` がシンボリックリンクであるファイルを拒否するようになりました。** サードパーティの各ファイルはリンクをたどって読み込まれていたため、コンポーネントが読み取り可能な任意のファイルのバイトをテンプレートとしてインストールできました。名指しされたファイルは、マニフェストのディレクトリ内の通常のファイルでなければなりません。
- **描画に置き換えられたモデル編集は、再入力すると送信されるようになりました。** ブラウザは各編集を直前に提案した値と比較していたため、拒否された値とそれを置き換える描画（リセットなど）の後に同じ値を再入力しても何も送信されませんでした。コントロールはエラーなくその値を表示する一方で、アイランドは別の値を保持していました。アイランドが適用する描画は次の編集と比較される基準になり、フィールドのダーティ状態はその描画がコントロールに与えた値と比較されます。
- **ツールチップは、表示されている場所で閉じられるようになりました。** コンテンツを覆う吹き出しは、これまでポインターかフォーカスを外すことによってしか閉じられず、これは WCAG 2.2 の達成基準 1.4.13 が認めないものでした。Escape キーは今後、どちらも動かすことなくそれを閉じ、そのトリガーを次にホバーまたはフォーカスすると再び表示されます。吹き出しはページにスクリプトがなくても変わらず表示されます。新しい `sn-tooltip` 要素だけがこの解除を担い、`live:add tooltip` がそれをインストールします。

### セキュリティ

- **ロックファイルが rustls の TLS 1.3 ハンドシェイク修正を取り込みました。** `rustls` 0.23.45 が 0.23.40 を置き換えます。0.23.40 は TLS 1.3 のハンドシェイクメッセージを暗号化レベルの境界を越えて受け入れていました （RUSTSEC-2026-0285、リリース当日に公開）。`aws-lc-rs` と `rustls-webpki` もあわせて更新されます。HTTP ファサードから Qdrant および決済アダプターまで、フレームワークのすべての HTTPS クライアントが修正済みのリリースを解決します。
- **Live ストリームは、それを開いたセッションとともに終了するようになりました。** 非同期メンバーシップは配信のたびに Gate に対して再認可されていましたが、セッションに対しては一度も確認されていませんでした。そのためログアウトしたブラウザや、セッションを失効させられたブラウザは、ストリーム自体が閉じるまでイベントを受け取り続けていました。あるノードでセッションが破棄されると、通常のログアウト、セッションの無効化、ID の再生成、「すべての端末からログアウト」のいずれでも、そのセッションがそこで開いたメンバーシップはすべて直ちに退会させられます。さらに配信は各メンバーシップのセッションをセッションストアに対して 10 秒に 1 回を上限として再確認するので、別のノードで破棄されたセッションもその間隔のうちにイベントを受け取らなくなります。
- **リクエストの `Cache-Control` ディレクティブを RenderCache が尊重するようになりました。** `no-cache` を付けたリクエストはストレージから `Age` 付きで応答され、`no-store` を付けたコールドなリクエストは次のリクエストのためにキャッシュを埋めていました。`no-store` はルックアップと公開の両方を迂回し、`no-cache` はルックアップを飛ばして新しいレンダリングで応答するようになりました。2026-09-13 の敵対的監査 (ASTRA-13) で発見。
- **スナップショットなしのレンダリングは決して公開されません。** スナップショットトランザクションを開けなかった場合、RenderCache は読み取りビューなしでレンダリングし、世代の再読み取りが一致すればそのまま公開していたため、複数行の同時書き込みと交錯したレンダリングが 2 つのデータベース状態の混在を保存し得ました。そのようなレンダリングは配信はされるものの保存されず （拒否理由 `snapshot_unavailable`）、その再構築リースは解放されます。2026-09-13 の敵対的監査 (ASTRA-08) で発見。
- **キャッシュされたレスポンスは `Content-Encoding` を再生します。** 事前圧縮されたボディが内容符号化なしで保存されていたため、ヒット時に gzip のバイト列がプレーンテキストとして返されていました。符号化はボディと一緒に保存され、すべてのヒットで再生されます。2026-09-13 の敵対的監査 (ASTRA-04) で発見。
- **HEAD リクエストが GET の表現を埋めることはなくなりました。** HEAD に対して何もレンダリングしないルートへのコールドな HEAD が、すべての GET が共有するキーの下に空のボディを保存していたため、以降の GET はゼロバイトを返していました。HEAD のミスはレンダリングどおりに配信され、保存されません （拒否理由 `head_render`）。2026-09-13 の敵対的監査 (ASTRA-03) で発見。
- **Live のスコープごとのサブスクリプション上限は、並行時にも維持されるようになりました。** 発行処理はスコープのサブスクリプション数を数え、ロックを解放し、認可処理を待ってから挿入していたため、同時に殺到したリクエストはすべてカウントを通過し、認可が返った後にすべて受け入れられて、公表されているスコープあたり 512 という上限を大きく超えることがありました。スロットはカウントと同じロックの下で予約され、すべてのエラー経路で解放され、レコードが確定した時点でそのレコードに引き渡されるようになりました。2026-09-13 の敵対的監査 (ASTRA-07) で発見。
- **Live のサブスクリプションは、その Gate が拒否した時点でイベントを受け取らなくなります。** ストリームの認可 Gate がプリンシパルを拒否するように再定義された後も、既存の非同期メンバーシップは新しく公開されたイベントを受け取り続けていました。配信が、サブスクリプション自身が保持していた認可メモをそれ自身と比較していたためです。新しいサブスクリプションは正しく拒否されていましたが、古いストリームはそうではありませんでした。ランタイムは、サブスクリプションが発行されたプリンシパルを記録し、配信のたびに Gate に再度問い合わせ、Gate がもはや許可しないメンバーシップを退去させるようになりました。2026-09-13 の敵対的監査 (ASTRA-01) で発見。
- **`transaction = "required"` を宣言する Live アクションは、登録時に拒否されるようになりました。** ホストのトランザクションポートは何もしないことが明記されているため、このポリシーは実際には提供されない原子性を約束していました。各書き込みは個別にコミットされ、後続の段階が失敗しても何もロールバックされませんでした。`LiveRegistry` は、ポートが本物のアンビエントトランザクションを導入するまで、そのようなコンポーネントに対して `RegistryErrorKind::RequiredTransactionUnsupported` で失敗します。このポリシーを持たないアクションは従来どおり登録できます。2026-09-13 の敵対的監査 (ASTRA-05) で発見。
- **キャッシュされたルートの名前付き接続への読み取りは、その接続にとどまるようになりました。** RenderCache のミスはプライマリデータベース上のスナップショットトランザクションの中でレンダリングされ、クエリのルーティングはクエリ自身の `on("name")` やモデルが宣言した接続よりもそのトランザクションを優先していたため、ルートをキャッシュに登録すると、そのコードがどのデータベースから読むかが変わっていました。テナント用や補助のデータベースへの読み取りが、プライマリの行を返したり、プライマリに存在しないテーブルで失敗したり、有効なキーの下に誤った内容を公開したりし得たのです。別の接続向けの読み取りは、アンビエントトランザクションの中でもその接続で実行されるようになり、スナップショットの外で読み取ったレンダリングは配信はされるものの保存されません （拒否理由 `foreign_connection_read`）。2026-09-13 の敵対的監査 (ASTRA-06) で発見。
- **データの書き込みと RenderCache の無効化が一緒にコミットされるようになりました。** オートコミット経路では、モデルの保存、クエリビルダーの書き込み、生の SQL 文が先に行を確定し、その後の 2 つ目のトランザクションで依存関係の世代を進めていました。この 2 つ目のトランザクションが失敗すると、行は永続化されたまま API はエラーを返し、その行に依存するキャッシュ済みページはすべて整合性チェックを通過し続けて書き込み前の内容を返していました。可視性や権限の変更、削除が、次に無効化が成功するまで見えないままになり得たのです。すべての書き込み終端は、行の書き込みと世代の前進をそのために開いた 1 つのトランザクションの中で実行するようになり、両方がコミットされるか、どちらもされないかのいずれかになります。RenderCache が存在すると判断した後に消えた台帳テーブルは、警告付きでスキップされる代わりに書き込みを失敗させます。台帳がプライマリにある名前付き接続向けの書き込みは、分離された世代の前進を維持します。その前進が失敗した場合、プロセスは前進が成功するまで保存済みエントリの配信を停止します。2026-09-13 の敵対的監査 (ASTRA-10) で発見。
- **ハンドラの `Vary` 契約は、RenderCache が保存する前に強制されるようになりました。** 独自のリクエストヘッダーによってボディを変え、それを `Vary` で宣言していたハンドラが、ルートポリシーだけから組み立てたキーの下に保存されていたため、最初のバリアントのボディが他のすべてのバリアントに返され、ヒット時には `Vary` ヘッダーも消えていました。そのヘッダーがユーザー、デバイス、実験ごとのコンテンツを選んでいた場合、あるリクエストのコンテンツが別のリクエストに届いていました。RenderCache は公開前にレスポンスの `Vary` を解析し、`*` またはポリシーがキーの次元として宣言していないフィールドを含む場合は保存を拒否するようになりました （拒否理由 `vary_undeclared`）。2026-09-13 の敵対的監査 (ASTRA-09) で発見。
- **ハンドラの `Cache-Control: no-store` を RenderCache が尊重するようになりました。** キャッシュに登録されたルートが、ハンドラが `no-store` と指定したレスポンスを保存し、ポリシー側の `public, max-age=60, s-maxage=60` を付けて再生していたため、ハンドラの「保存しない」という指示はサーバー側で無視され、下流のすべてのブラウザーとプロキシに対して書き換えられていました。適格性チェックはレスポンスの `Cache-Control` を読み、`no-store` トークンがあれば保存を拒否するようになりました （拒否理由 `no_store_directive`）。拒否されたレスポンスは、ハンドラが構築したとおりにそのまま送出されます。2026-09-13 の敵対的監査 (ASTRA-02) で発見。
- **レスポンスごとの CSP nonce がキャッシュから再生されることはなくなりました。** レンダリングのたびに nonce を生成して `Content-Security-Policy` で宣言していた公開ページが、通常の完全エントリとして保存されていたため、以降のすべてのキャッシュヒットがヘッダーとインラインスクリプトの両方に最初のレンダリングの nonce を載せていました。nonce は 1 つのレスポンスにおけるインラインスクリプトの認可トークンであり、それを再生することは、ページを取得できる誰にでもそのトークンを読めるようにすることを意味します。RenderCache はそのようなレスポンスの保存を拒否するようになりました （拒否理由 `nonce_source_policy`）。ハッシュベースのポリシーは従来どおりキャッシュされ、ステッチされた Live ドキュメントは引き続きヒットごとに新しい nonce を発行します。2026-09-13 の敵対的監査 (ASTRA-12) で発見。
- **キャッシュされたレスポンスは、分離と実行に関わるヘッダーを保持します。** RenderCache はレスポンスの `Content-Disposition`、`Cross-Origin-Opener-Policy`、`Cross-Origin-Embedder-Policy`、`Cross-Origin-Resource-Policy`、`Permissions-Policy`、`X-Frame-Options` をどこにも保存していなかったため、キャッシュヒットは同じバイト列をそれらなしで返していました。最初のリクエストでは添付ファイルとしてダウンロードされた HTML エクスポートが、2 回目にはアプリケーションのオリジンの下でインライン表示され、含まれていたマークアップが同一オリジンの権限で実行されていました。この 6 つのヘッダーは、保存された表現からバイト単位でそのまま再生されるようになりました。2026-09-13 の敵対的監査 (ASTRA-11) で発見。
- **ロックファイルから、不健全なリリース1つと取り下げ済みリリース3つが除かれました。** `event-listener` 5.4.2 が 5.4.1 を置き換えます。5.4.1 のスタック割り当てリスナーは無条件に `Send`/`Sync` で、安全なコードのまま `!Send` なタグがスレッド境界を越えられました（RUSTSEC-2026-0221）。Suprnova の依存関係はタグなしのイベントしか使わないので、この不健全な経路がここで実行されることはありませんでした。`spin` 0.9.9 と 0.10.1、`chacha20` 0.10.2 は、公開者が取り下げたリリースを置き換え、`concurrent-queue` はツリーから完全に消えます。タグの `Cargo.lock` は引き続き以前のバージョンを解決します。

## 2.0.1 - 2026-09-12

### 修正

- **データベースインデックス付きの `redis://` URL が、どこでもそのデータベースを選択するようになりました。** キュードライバーとファンアウトブロードキャストハブは、直接の redis 接続のかたわらにそれぞれ sea-streamer のプロデューサーを持っていますが、sea-streamer は URL のパスから論理データベースを読み取りません。そのため `redis://host:6379/3` に向けたキューは、コンシューマー側がデータベース `3` で待っている間、ジョブをデータベース `0` に積んでいて、ファンアウトハブに至っては最初から `0` 上で動作していました。どちらも今は、redis クライアントが適用するのと同じ解析規則で、インデックスを明示的に引き渡します。パスのない URL はこれまでどおりデータベース `0` を選択するので、一般的な形には何も変わりません。
...
- **2.0.0 の章の6言語ミラーが、マニュアル自身の慣例に従うようになりました。** 1.3.7 以降に翻訳された章は、日本語と簡体字中国語の文を文の途中で折り返し（余分な空白として描画される1,653か所の改行）、59の用語をその言語の他の章とは別の訳語で表し、繰り返し現れる9つの見出しを二通りに訳し、2つの引用符を閉じないままにしていました。これらすべてが古い章に揃えられ、どの言語の読者もマニュアル全体で一つの語彙と途切れない文を目にします。
