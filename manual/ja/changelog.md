...
- **チェックボックスグループがチェックされた値のリストを提案するようになりました。** ランタイムはすべてのチェックボックスを真偽値として読んでいたため、ボックスの状態が揃わないグループは送信できませんでした。複数のチェックボックスが結び付くフィールドは、チェックされた値をドキュメント順で提案するようになりました。単独のチェックボックスは真偽値のままです。
