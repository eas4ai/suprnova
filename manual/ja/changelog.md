...
- **ノードどうしが、ある世代が今動いたことを互いに伝えられます。** `RENDER_CACHE_HINTS` は、ある前進がちょうど触れた依存のダイジェストを運ぶRedisのpub/subのチャンネルを有効にします。`redis` のプロファイルではデフォルトで有効、ほかの2つでは無効であり、キャッシュ自身と同じ `RENDER_CACHE_REDIS_URL` と `RENDER_CACHE_REDIS_PREFIX` に相乗りします。ヒントの唯一の力は、ノードに、自分のリースが促すよりも早く再検証させることです: リースを延長したり作ったりすること、エントリが最新だと証明すること、ヒットが今も行う台帳の読み取りを迂回すること、権威のエポックに触れることは、決してできません。これが、このチャンネルが設計上認証されていない理由であり、到達できないキャッシュのティアとは違って、到達できないヒントのエンドポイントが起動を拒まない理由です。ヒントは瞬間を運ばないため、ノード間の時計のずれについての仮定は要りません。ヒントを無効にしたデプロイ、チャンネルが死んでいるデプロイ、そしてヒントを一度も持たなかったデプロイは、同じエントリを配信し、同じ再構築を受け入れます。違うのは、再検証の瞬間だけです。
