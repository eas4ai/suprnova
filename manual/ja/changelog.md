...
- **スキャフォルドされたアプリケーションが、メールアドレスを検証し、パスワードをリセットするようになりました。** 登録は、検証メールを送らずにユーザーを作成してログインさせ、メール検証やパスワード回復を提供するルートはひとつもありませんでした。生成された `User` は既に `MustVerifyEmail` と `CanResetPassword` を実装し、`auth_flow_tokens` マイグレーションも既に同梱されていたにもかかわらず、です。登録は今では `EmailVerification::send_link` を通じて検証リンクをメールで送り、`/verify-email` へ進みます。そこでは案内を表示し、リンクを再送し、`/verify-email/verify` でログイン中の所有者に限ってリンクを消費します。`/forgot-password` は検証済みのアドレスへリセットリンクをメールで送り（未知のアドレスや未検証のアドレスは同じ応答を受け取り、メールは届きません）、`/reset-password` は `PasswordReset::complete_with_outcome` を通じてパスワードをローテーションし、そのアカウントの他のセッションやremember-meトークンを取り消せなかった間は完了を拒否します。Vue、React、Svelteの各スターターは `ForgotPassword`、`ResetPassword`、`VerifyEmail` のページと、ログインページの「Forgot your password?」リンクを同梱します。リンクは `url::to` で組み立てられるので、`APP_URL` はユーザーがアプリケーションへ到達するアドレスを指していなければならず、登録には動作するメールトランスポートが必要です: スキャフォルドが書く `.env` は `MAIL_DRIVER=smtp` をポート1025のローカルのキャッチャー（composeファイルのMailpit）へ向けています。あるいは `MAIL_DRIVER=log` を設定すれば、各メッセージがリンクごとサーバーログへ出力されます。
