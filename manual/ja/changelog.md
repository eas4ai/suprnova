...
- **Magnetarは、クレデンシャルおよびセッションの変更を、認証済みアクターとアカウント認証エポックで柵囲いするようになりました。** パスワード、パスキー、リンク済みアカウント、二要素、不透明セッション、JWT、remember、OAuth、およびデバイス認可の書き込みは、古いまたは失効済みのアクターを拒否します。未確認アカウントで最初に成功したパスワードリセット、マジックリンク、またはOAuth確認済みメール証明は、エポックを進め、暫定クレデンシャル、セッション、remember状態、および占有者のTOTP登録を原子的に削除します。確認済みアカウントは、パスワードリセット時に正当なクレデンシャルを保持します。メール確認には認証済みトークン所有者が必要であり、OAuthは未確認の既存アカウントをメールだけから自動リンクすることはありません。
