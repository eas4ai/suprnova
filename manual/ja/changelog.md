...
- **マニュアルが6言語で提供されるようになりました。** `manual/es/`、`manual/fr/`、
  ...
  意図的なスタイルの違いが1つあります: 英語のマニュアルがemダッシュを使う箇所で、翻訳はASCIIのハイフンを使います。これは翻訳が執筆されたサイトのハウススタイルに合わせたものです。散文のみで、コードや出力は一切違いません。
