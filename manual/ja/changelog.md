...
- **リクエストの `Cache-Control` ディレクティブを RenderCache が尊重するようになりました。** `no-cache` を付けたリクエストはストレージから `Age` 付きで応答され、`no-store` を付けたコールドなリクエストは次のリクエストのためにキャッシュを埋めていました。`no-store` はルックアップと公開の両方を迂回し、`no-cache` はルックアップを飛ばして新しいレンダリングで応答するようになりました。2026-09-13 の敵対的監査 (ASTRA-13) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **スナップショットなしのレンダリングは決して公開されません。** スナップショットトランザクションを開けなかった場合、RenderCache は読み取りビューなしでレンダリングし、世代の再読み取りが一致すればそのまま公開していたため、複数行の同時書き込みと交錯したレンダリングが 2 つのデータベース状態の混在を保存し得ました。そのようなレンダリングは配信はされるものの保存されず (拒否理由 `snapshot_unavailable`)、その再構築リースは解放されます。2026-09-13 の敵対的監査 (ASTRA-08) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **キャッシュされたレスポンスは `Content-Encoding` を再生します。** 事前圧縮されたボディが内容符号化なしで保存されていたため、ヒット時に gzip のバイト列がプレーンテキストとして返されていました。符号化はボディと一緒に保存され、すべてのヒットで再生されます。2026-09-13 の敵対的監査 (ASTRA-04) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **HEAD リクエストが GET の表現を埋めることはなくなりました。** HEAD に対して何もレンダリングしないルートへのコールドな HEAD が、すべての GET が共有するキーの下に空のボディを保存していたため、以降の GET はゼロバイトを返していました。HEAD のミスはレンダリングどおりに配信され、保存されません (拒否理由 `head_render`)。2026-09-13 の敵対的監査 (ASTRA-03) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **Live のスコープごとのサブスクリプション上限は、並行時にも維持されるようになりました。** 発行処理はスコープのサブスクリプション数を数え、ロックを解放し、認可処理を待ってから挿入していたため、同時に殺到したリクエストはすべてカウントを通過し、認可が返った後にすべて受け入れられて、公表されているスコープあたり 512 という上限を大きく超えることがありました。スロットはカウントと同じロックの下で予約され、すべてのエラー経路で解放され、レコードが確定した時点でそのレコードに引き渡されるようになりました。2026-09-13 の敵対的監査 (ASTRA-07) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **Live のサブスクリプションは、その Gate が拒否した時点でイベントを受け取らなくなります。** ストリームの認可 Gate がプリンシパルを拒否するように再定義された後も、既存の非同期メンバーシップは新しく公開されたイベントを受け取り続けていました。配信が、サブスクリプション自身が保持していた認可メモをそれ自身と比較していたためです。新しいサブスクリプションは正しく拒否されていましたが、古いストリームはそうではありませんでした。ランタイムは、サブスクリプションが発行されたプリンシパルを記録し、配信のたびに Gate に再度問い合わせ、Gate がもはや許可しないメンバーシップを退去させるようになりました。2026-09-13 の敵対的監査 (ASTRA-01) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **`transaction = "required"` を宣言する Live アクションは、登録時に拒否されるようになりました。** ホストのトランザクションポートは何もしないことが明記されているため、このポリシーは実際には提供されない原子性を約束していました。各書き込みは個別にコミットされ、後続の段階が失敗しても何もロールバックされませんでした。`LiveRegistry` は、ポートが本物のアンビエントトランザクションを導入するまで、そのようなコンポーネントに対して `RegistryErrorKind::RequiredTransactionUnsupported` で失敗します。このポリシーを持たないアクションは従来どおり登録できます。2026-09-13 の敵対的監査 (ASTRA-05) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **キャッシュされたルートの名前付き接続への読み取りは、その接続にとどまるようになりました。** RenderCache のミスはプライマリデータベース上のスナップショットトランザクションの中でレンダリングされ、クエリのルーティングはクエリ自身の `on("name")` やモデルが宣言した接続よりもそのトランザクションを優先していたため、ルートをキャッシュに登録すると、そのコードがどのデータベースから読むかが変わっていました。テナント用や補助のデータベースへの読み取りが、プライマリの行を返したり、プライマリに存在しないテーブルで失敗したり、有効なキーの下に誤った内容を公開したりし得たのです。別の接続向けの読み取りは、アンビエントトランザクションの中でもその接続で実行されるようになり、スナップショットの外で読み取ったレンダリングは配信はされるものの保存されません (拒否理由 `foreign_connection_read`)。2026-09-13 の敵対的監査 (ASTRA-06) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **データの書き込みと RenderCache の無効化が一緒にコミットされるようになりました。** オートコミット経路では、モデルの保存、クエリビルダーの書き込み、生の SQL 文が先に行を確定し、その後の 2 つ目のトランザクションで依存関係の世代を進めていました。この 2 つ目のトランザクションが失敗すると、行は永続化されたまま API はエラーを返し、その行に依存するキャッシュ済みページはすべて整合性チェックを通過し続けて書き込み前の内容を返していました。可視性や権限の変更、削除が、次に無効化が成功するまで見えないままになり得たのです。すべての書き込み終端は、行の書き込みと世代の前進をそのために開いた 1 つのトランザクションの中で実行するようになり、両方がコミットされるか、どちらもされないかのいずれかになります。RenderCache が存在すると判断した後に消えた台帳テーブルは、警告付きでスキップされる代わりに書き込みを失敗させます。台帳がプライマリにある名前付き接続向けの書き込みは、分離された世代の前進を維持します。その前進が失敗した場合、プロセスは前進が成功するまで保存済みエントリの配信を停止します。2026-09-13 の敵対的監査 (ASTRA-10) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **ハンドラーの `Vary` 契約は、RenderCache が保存する前に強制されるようになりました。** 独自のリクエストヘッダーによってボディを変え、それを `Vary` で宣言していたハンドラーが、ルートポリシーだけから組み立てたキーの下に保存されていたため、最初のバリアントのボディが他のすべてのバリアントに返され、ヒット時には `Vary` ヘッダーも消えていました。そのヘッダーがユーザー、デバイス、実験ごとのコンテンツを選んでいた場合、あるリクエストのコンテンツが別のリクエストに届いていました。RenderCache は公開前にレスポンスの `Vary` を解析し、`*` またはポリシーがキーの次元として宣言していないフィールドを含む場合は保存を拒否するようになりました (拒否理由 `vary_undeclared`)。2026-09-13 の敵対的監査 (ASTRA-09) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **ハンドラーの `Cache-Control: no-store` を RenderCache が尊重するようになりました。** キャッシュに登録されたルートが、ハンドラーが `no-store` と指定したレスポンスを保存し、ポリシー側の `public, max-age=60, s-maxage=60` を付けて再生していたため、ハンドラーの「保存しない」という指示はサーバー側で無視され、下流のすべてのブラウザーとプロキシに対して書き換えられていました。適格性チェックはレスポンスの `Cache-Control` を読み、`no-store` トークンがあれば保存を拒否するようになりました (拒否理由 `no_store_directive`)。拒否されたレスポンスは、ハンドラーが構築したとおりにそのまま送出されます。2026-09-13 の敵対的監査 (ASTRA-02) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **レスポンスごとの CSP nonce がキャッシュから再生されることはなくなりました。** レンダリングのたびに nonce を生成して `Content-Security-Policy` で宣言していた公開ページが、通常の完全エントリとして保存されていたため、以降のすべてのキャッシュヒットがヘッダーとインラインスクリプトの両方に最初のレンダリングの nonce を載せていました。nonce は 1 つのレスポンスにおけるインラインスクリプトの認可トークンであり、それを再生することは、ページを取得できる誰にでもそのトークンを読めるようにすることを意味します。RenderCache はそのようなレスポンスの保存を拒否するようになりました (拒否理由 `nonce_source_policy`)。ハッシュベースのポリシーは従来どおりキャッシュされ、ステッチされた Live ドキュメントは引き続きヒットごとに新しい nonce を発行します。2026-09-13 の敵対的監査 (ASTRA-12) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **キャッシュされたレスポンスは、分離と実行に関わるヘッダーを保持します。** RenderCache はレスポンスの `Content-Disposition`、`Cross-Origin-Opener-Policy`、`Cross-Origin-Embedder-Policy`、`Cross-Origin-Resource-Policy`、`Permissions-Policy`、`X-Frame-Options` をどこにも保存していなかったため、キャッシュヒットは同じバイト列をそれらなしで返していました。最初のリクエストでは添付ファイルとしてダウンロードされた HTML エクスポートが、2 回目にはアプリケーションのオリジンの下でインライン表示され、含まれていたマークアップが同一オリジンの権限で実行されていました。この 6 つのヘッダーは、保存された表現からバイト単位でそのまま再生されるようになりました。2026-09-13 の敵対的監査 (ASTRA-11) で発見。`v2.0.1` タグの後に main に取り込まれました。

- **ロックファイルから、不健全なリリース1つと取り下げ済みリリース3つが除かれました。** `event-listener` 5.4.2 が 5.4.1 を置き換えます。5.4.1 のスタック割り当てリスナーは無条件に `Send`/`Sync` で、安全なコードのまま `!Send` なタグがスレッド境界を越えられました（RUSTSEC-2026-0221）。Suprnova の依存関係はタグなしのイベントしか使わないので、この不健全な経路がここで実行されることはありませんでした。`spin` 0.9.9 と 0.10.1、`chacha20` 0.10.2 は、公開者が取り下げたリリースを置き換え、`concurrent-queue` はツリーから完全に消えます。これは `v2.0.1` タグの後に main に入ったものです。タグの `Cargo.lock` は引き続き以前のバージョンを解決します。
