...
2. **アンビエントな `CURRENT_TX`** - クロージャのタスクスコープのために、`DB::transaction` / `DB::transaction_with_attempts` によってインストールされます。
   `on(name)` またはモデルが宣言した接続で別の接続を指名する読み取りは、引き続きその接続で実行されます。トランザクションは 1 つのデータベースに固定されているため、読み取りがそこで参加することはできず、経路を変えるとコードがどのデータベースから読むかが変わってしまいます（監査所見 ASTRA-06、2026-09-13）。書き込みはトランザクション上にとどまります。原子性は接続をまたいで分割してはならないからです。
