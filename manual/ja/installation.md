...
`suprnova new` は完全なプロジェクト - バックエンド + 選択したフロントエンド
+ Vite 設定 + 認証マイグレーション + サンプルルート - をスキャフォルドします。デフォルトではインタラクティブです：
