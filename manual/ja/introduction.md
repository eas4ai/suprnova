...
| **認証** | ステートフルセッション、不透明なユーザー ID、複数の認証ガード、Eloquent＋データベースプロバイダー、パスワードハッシング（bcrypt＋argon2）、ポリシーマクロ、ゲート、メール検証、パスワードリセット、ブルートフォーススロットリング、TOTP 2FA、ログイン状態の保存、torii 統合経由の OAuth |
...
| **メール & 通知** | `Mailable` トレイト、SMTP/SES/Mailgun/Postmark/SendGrid/Resend 用ドライバー（テスト用のメモリ内＆ログを含む）、メール/データベース/ブロードキャスト/webpush チャネル付き `Notifiable` |
...
フレームワークは本番環境レベルのスコープとテストされています。現在のゴール時点で：
