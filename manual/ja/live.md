...
`popover` 属性により、サポートする基準は Chrome と Edge 114、Firefox 128、Safari 17 になります。CSS のアンカー配置があるところでは、ポップオーバーとメニューはトリガーの下に置かれ、それ以外ではブラウザーが中央に配置します。アコーディオンの単一オープンモードは `details name` に依存しており、古いサポート対象バージョンではそれぞれ独立したディスクロージャーとして扱われます。ツールチップは、ポインターがトリガーから吹き出しへ移動する間も開いたままなので、そのテキストを読んだり選択したりできます。
