...
送り、セッショントークンは持ちません。CSRF ミドルウェアをオリジン検証するよう設定
すると、同一オリジンの Live リクエストはステートレス CSRF 判定で通過し、クロスサイトや
ヘッダーなしのリクエストはトークン検証にフォールバックして拒否されます:

```rust
global_middleware!(CsrfMiddleware::new().with_origin_policy(OriginPolicy::SameOriginOnly));
```

匿名の訪問者は公開シードをレンダリングできますが、アクションは実行できません。
ガードの `AuthMiddleware` が、エンジンの処理より前に `401` で応答します。
アイデンティティ結合アイランドにはセッションとプリンシパルが必要です。リゾルバーが
テナントを名指しするたびに、テナントはアイランドのスコープに結合されます。すべての
...
登録済みハンドラーに配信されます。ポーリングは通常の新規レンダリングなので、
トランスポートが使えなくても何も失われません。
