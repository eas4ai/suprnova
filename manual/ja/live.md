...
- `#[public]` フィールドはレンダリングされ、署名付きスナップショットに含まれます。
  ...
  モデルフィールドは `#[model(debounce = 250)]` のように属性でタイミングを宣言します。デバウンスは `live:model.debounce.<n>ms` が受け付ける 100、250、500 ミリ秒のいずれかで、それ以外の値はコンパイルに失敗します。
  `live:submit` で送信するフォームは、内部のすべてのモデルコントロールを 1 つのリクエストで提案し、そのリクエストはアクションに加えて最大 127 フィールドを運びます。それより大きなフォームは `live:check` が拒否します。
  フィールドがデコードできない提案、たとえば `u64` フィールドへの空の数値は、そのフィールドのバリデーションエラーになります。アクションは実行されず、フィールドは値を保ち、`live:error` がエラーを表示します。
...
`live:key` はモーフをまたぐ要素の安定した同一性を名付け、テンプレートが書く唯一のキー属性です。ランタイムはモーフの同一性、`live:preserve.self` のようなモーフ制御、ブラウザー状態を保つスコープのためにそれを読みます。`data-suprnova-live-key` はエンジンが描画するルート上でのエンジン自身の表記です。アイランド内の `live:key` の値と要素 ID は、ランタイムがモーフのたびに検査する 1 つのアルファベットを共有します。先頭は ASCII の英字または数字で、その後に英字、数字、`_`、`-`、`.`、`:` が続き、最大 128 バイトで、それぞれアイランド内で一意です。`live:check` は、この範囲外のリテラルのキーや ID と、最初の項目より後のすべての項目が繰り返すことになる、ループ内のリテラル ID を拒否します。
...
`live:add` は書き込むすべてのファイルのダイジェストを記録します。そのため後の実行では、一度も編集していないファイルはライブラリが変更したときに置き換えられ、編集したファイルは保持されてその旨が報告されます。`--force` は編集済みのファイルも置き換えます。サードパーティのコンポーネントは `--manifest` で独自のマニフェストから、独自のルートの下にインストールされ、マニフェストが名指しする各ファイルは、マニフェストのディレクトリ内の通常のファイルでなければならず、シンボリックリンクであってはなりません。ビューからマクロを呼び出し、`try_live_ui_assets()` でスタイルシートとスクリプトを配信し、ドキュメントからリンクします。
...
`try_live_ui_assets()` はリクエストのたびに、アプリケーションのベースパス配下の `templates/suprnova-ui/` からファイルを読み込みます。そのため、このディレクトリをバイナリと一緒に配布し、それを含むディレクトリからアプリケーションを起動するか、`APP_BASE_PATH` にそのディレクトリを設定してください。ディレクトリを読み込めない場合、アプリケーションは起動を拒否し、そのディレクトリを示します。
...
各値コントロールはアイランドの現在の値を受け取るため、ページにはその値が表示され、何も変更しなかった送信はその値を変えずに送り返します。入力、テキストエリア、数値入力、スライダー、検索入力、日付ピッカーには `value=`、チェックボックスとスイッチには `checked=`、セレクト、ラジオグループ、チェックボックスグループには `selected=` を使い、チェックボックスグループはチェックされた値のリストを受け取ります。パスワードとワンタイムコードは値を決して描画しません。ラジオグループとチェックボックスグループの入力は値をキーとするため、ユーザーがまだ送信していない選択は再描画を生き延びます。チェックボックスグループは、選択肢の数に関わらずチェックされた値のリストとして提案され、複数のチェックボックスが結び付く任意のフィールドも同様です。単一のチェックボックスは真偽値として提案されます。リセットに応答する描画のように、ユーザーが入力した内容を置き換えなければならない描画は `authority=` にシーケンス番号を渡し、次の描画は何も渡しません。
...
`popover` 属性により、サポートする基準は Chrome と Edge 114、Firefox 128、Safari 17 になります。CSS のアンカー配置があるところでは、ポップオーバーとメニューはトリガーの下に置かれ、それ以外ではブラウザーが中央に配置します。アコーディオンの単一オープンモードは `details name` に依存しており、古いサポート対象バージョンではそれぞれ独立したディスクロージャーとして扱われます。ツールチップは、ポインターがトリガーから吹き出しへ移動する間も開いたままなので、そのテキストを読んだり選択したりでき、Escape キーはポインターやフォーカスを動かすことなく、表示されている場所でそれを閉じます。そのトリガーを次にホバーまたはフォーカスすると、再び表示されます。閉じる処理はベンダリングされた `sn-tooltip` 要素だけが担い、それを読み込まないページでも吹き出しはホバー時とフォーカス時に変わらず表示されます。

続いてフィードバックファミリーとナビゲーションファミリーです。フィードバックはアラート、スケルトン、スピナー、プログレス、空状態、そしてフラッシュ領域を隣に持つトースト領域です。それぞれはサーバーまたはランタイムがすでに持つ状態を表示します。アラートはバリアントからロールを選び、色だけでなく記号と非表示のラベルで各バリアントを示します。スピナーやスケルトンは `live:loading.show` で登録済みアクションに結び付けられ、非表示で出力されるため、ランタイムが自身の遅延の後に表示し最小時間を超えて保持し、速いアクションで点滅することはありません。プログレスはラベルとテキストの読み上げを持つネイティブの `progress` 要素で、確定的な作業のときだけ値を持ちます。空状態は理由（空、結果なし、権限なし、切断）をサーバーで描画された状態から取り、呼び出し側が描画する場合にだけ次のアクションを提示します。トーストは丁寧なステータス領域から一度だけ通知し、フォーカスを奪いません。ベンダリングされた `sn-toast-region` 要素はトーストをタイムアウトさせ、ポインターがトーストのいずれかの部分の上にある間やフォーカスがその内側にある間は一時停止し、同時に表示する数を制限し、閉じるボタンに応答します。各トーストはキー付きで保持されるため、閉じたトーストはモーフをまたいでも閉じたままです。重大なエラーはアラートにも置くべきで、トーストがその唯一の表示面になることはありません。トーストはループの中で描画されるため、そのキーは `live_key` フィルターを通り、トーストをマウントするアイランドは `pub mod filters { pub use suprnova::view::filters::live_key; }` でこのフィルターを公開します。`live_key` は、メールアドレスのようにキーのアルファベット外の値に対してアイランドの描画を失敗させます。そのようなデータには `live_key_digest` でキーを付けてください。このフィルターは任意の値をアルファベット内の安定したキーに変換し、同じように `suprnova::view::filters` からエクスポートされています。フラッシュ領域は前のリクエストがセッションに残したものを一度だけ描画します。
...
`render_chart` は、絶対値が 1e9 を超える値を描画せずにエラーを返します。そのような値は、レンダラーの軸の計算が扱える範囲を超えるためです。
