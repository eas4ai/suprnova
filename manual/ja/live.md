...
各値コントロールはアイランドの現在の値を受け取るため、ページにはその値が表示され、何も変更しなかった送信はその値を変えずに送り返します。入力、テキストエリア、数値入力、スライダー、検索入力、日付ピッカーには `value=`、チェックボックスとスイッチには `checked=`、セレクト、ラジオグループ、チェックボックスグループには `selected=` を使い、チェックボックスグループはチェックされた値のリストを受け取ります。パスワードとワンタイムコードは値を決して描画しません。ラジオグループとチェックボックスグループの入力は値をキーとするため、ユーザーがまだ送信していない選択は再描画を生き延びます。複数のチェックボックスが結び付くフィールドはチェックされた値のリストとして提案され、チェックボックスが 1 つだけのグループは真偽値として提案されます。リセットに応答する描画のように、ユーザーが入力した内容を置き換えなければならない描画は `authority=` にシーケンス番号を渡し、次の描画は何も渡しません。
