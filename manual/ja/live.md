...
`live:key` はモーフをまたぐ要素の安定した同一性を名付け、テンプレートが書く唯一のキー属性です。ランタイムはモーフの同一性、`live:preserve.self` のようなモーフ制御、ブラウザー状態を保つスコープのためにそれを読みます。`data-suprnova-live-key` はエンジンが描画するルート上でのエンジン自身の表記です。
