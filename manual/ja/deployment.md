...
Dockerfile がビルドするバイナリは、フレームワークの `testing` 機能を一度も持ち運びません。テストスイートが必要とする暗号化キーのインストーラー、ストレージのフェイク、RenderCache のテストフックは、コンパイルから外されています。スキャフォルドされたあなたの `Cargo.toml`（そして
Suprnova リポジトリ自身のドッグフードアプリ）は、2 つの依存関係エントリで構成によりこの形を取ります。
...
`cmd/` の中の何も、バイナリターゲットのためにそれをオンにするべきではありません。

フレームワークのリポジトリは、コンパイルに失敗することだけを仕事とする小さなクレートで、これが成り立つことを証明します。それは `testing` の背後に隠されたフレームワークのあらゆる項目をパス経由で参照し、リリースゲートは、機能なしでそれらを参照するとコンパイルエラーになり、機能ありで参照すると成功することを検査します。
