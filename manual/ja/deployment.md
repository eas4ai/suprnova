...
破壊的なマイグレーションをロールするか、インシデント用にトラフィックを静止させるには：
...
`down` はミドルウェアがすべてのリクエストで読む保守マーカーを書き込みます。リクエストは 503（`--status` で設定可能）と提供されたメッセージを取得しますが、`--except` のパスと秘密を含むリクエストは除きます。`up` はマーカーを削除します。
