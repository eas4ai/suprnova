...
JSON-lines report from the framework's hidden tooling helper, so it never
needs a framework dependency of its own. Build output stays on stderr;
anything unexpected on stdout fails the command closed.
