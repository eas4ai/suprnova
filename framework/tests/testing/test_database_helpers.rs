...
async fn execute_unprepared_and_fetch_round_trip() {
    ...
    db.execute_unprepared("CREATE TABLE probe (id INTEGER PRIMARY KEY, name TEXT NOT NULL)")
        .await
        .unwrap();
    db.execute_unprepared("INSERT INTO probe (id, name) VALUES (1, 'alpha'), (2, 'beta')")
        .await
        .unwrap();

    let row = db
        .fetch_one(
            "SELECT name FROM probe WHERE id = ?",
            vec![Value::from(1i64)],
        )
        .await
        .unwrap();
    ...
    let rows = db
        .fetch_all("SELECT id FROM probe ORDER BY id", vec![])
        .await
        .unwrap();
...
async fn fetch_one_errors_when_no_rows() {
    let db = TestDatabase::sqlite_memory().await.unwrap();
    db.execute_unprepared("CREATE TABLE empty (id INTEGER)")
        .await
        .unwrap();
    let err = db
        .fetch_one("SELECT id FROM empty", vec![])
        .await
        .unwrap_err();
