...
//! default, PostgreSQL and MySQL through the `#[ignore]`d live tests at the
...
//!
//! The last section proves what one adapter cannot: that two nodes sharing
//! one database agree about who may publish and what a restart finds. Those
//! three proofs are the parent module's, called from here with this tier's
//! handles and from `redis` with the other tier's.
...
use super::{
    acquired, assert_a_restarted_node_reads_the_records_the_previous_one_left,
    ...
    conformance_ledger, instance_row_count, row_count,
};
...
// proves what an adapter alone cannot: that two nodes sharing one backend
// agree about who may publish, and that a restart finds what the process
// before it left.
//
// Two nodes are two adapter handles and two coordinators over one database,
...
// something a test process can have. The bodies themselves are the parent
// module's, so the Redis tier answers the same three proofs rather than
// three that merely read alike.
...
async fn two_nodes_publish_one_entry_and_the_node_that_bypassed_publishes_nothing() {
    ...
    // Counted in SQL rather than through the store: the shared proof reads
    // the store's own inspection, and this says the same thing about the
    // table underneath it.
...
async fn a_restarted_node_reads_the_instance_records_the_previous_one_left() {
    ...
    assert_a_restarted_node_reads_the_records_the_previous_one_left(&key, expires_at, || async {
        SqlInstanceRecordStore::new()
    })
    .await;
}
