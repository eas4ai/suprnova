...
pub const FORM_DOCUMENT_PATH: &str = "/dogfood/form";
pub const FORM_DOCUMENT_KEY: &str = "dogfood-form";

/// A form island whose model fields decode strictly: a seat count and a
/// topic list, with a count of the saves that ran.
#[derive(LiveComponent)]
#[live(name = "tests.dogfood-form", view = "live/tests/dogfood-form.html")]
pub struct DogfoodForm {
    #[model]
    seats: u64,
    #[model]
    topics: Vec<String>,
...
/// `fixture` with the form island registered beside the counter.
pub fn form_fixture() {
    fixture();
    App::singleton(
        LiveRegistry::builder()
            .register::<DogfoodCounter>()
            .expect("register dogfood counter")
            .register::<DogfoodForm>()
            .expect("register dogfood form")
            .build(),
    );
}
...
/// The guarded dogfood router with the form island beside the counter. A
/// process binds one mount catalog, so the form's tests own their binary.
pub fn build_form_router() -> Router {
    with_form_route(build_router_with(live_guard))
}

/// Adds the form island's document at `FORM_DOCUMENT_PATH` to `router`.
fn with_form_route(router: Router) -> Router {
    let mount =
        LiveMount::<DogfoodForm>::public_seed(FORM_DOCUMENT_PATH, "form", FORM_DOCUMENT_KEY)
            .expect("declare dogfood form mount");
    let handler_mount = mount.clone();
    let router: Router = router
        .get(FORM_DOCUMENT_PATH, move |request: Request| {
            let mount = handler_mount.clone();
            async move {
                let result: Result<HttpResponse, FrameworkError> = async {
                    let mut document = LiveDocument::from_request(&request).map_err(|error| {
    ...
    router
        .try_live_mount(&mount)
        .expect("register dogfood form mount")
}
...
pub fn form_action_request(
    ...
    let (base_revision, snapshot) = match &spec.snapshot["body"]["revision"] {
        Value::String(revision) => (
            revision.clone(),
            json!({"envelope": spec.snapshot, "kind": "instance"}),
        ),
    ...
    let mut operations: Vec<Value> = proposals
        .as_object()
        .expect("proposals are an object")
        .keys()
    ...
    let body = serde_json::to_vec(&json!({
        ...
        "component": "tests.dogfood-form",
        "correlation_id": "MDEyMzQ1Njc4OTo7PD0-Pw",
        "extensions": {"x_suprnova_live_document_key_v1": FORM_DOCUMENT_KEY},
    ...
    .expect("encode Live form action request");
    ...
    builder
        ...
        .expect("build Live form action request")
}
