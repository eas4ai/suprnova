...
case ${live_root} in
    "${workspace_root}"/*) ;;
    *)
        printf 'action budget: Live root is outside the Suprnova workspace (%s)\n' \
            "${live_root}" >&2
        exit 70
        ;;
esac
...
result_path=${SUPRNOVA_LIVE_BENCH_RESULT:-"${live_root}/benchmarks/action-budget-v1.json"}
