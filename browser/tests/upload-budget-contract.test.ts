...
import {
  ...
  uploadServerEvidenceFromProcessRuns,
...
function evidence(): Record<string, unknown> {
  ...
  server["runs"] = [
    {
      artifactSha256: SHA256,
      environment: structuredClone(server["environment"]),
      measurements: structuredClone(server["measurements"]),
      methodology: { measuredSamples: 40, warmupIterations: 50 },
      processId: 10_001,
      runIndex: 1,
      workload: structuredClone(server["workload"]),
    },
  ];
  return candidate;
}
...
function qualifyThreeRuns(value: Record<string, unknown>): void {
  ...
  const serverFirst = server.runs[0];
  if (serverFirst === undefined) throw new Error("fixture_server_run_missing");
  server.runs = [1, 2, 3].map((runIndex) => ({
    ...structuredClone(serverFirst),
    environment: structuredClone(server.environment),
    processId: 10_000 + runIndex,
    runIndex,
  }));
  server.methodology.independentRuns = 3;
  server.methodology.measuredSamples = 120;
}
...
function setServerRunP95s(value: Record<string, unknown>, p95Values: readonly number[]): void {
  const server = value["server"] as {
    measurements: Record<string, unknown>;
    runs: { measurements: Record<string, unknown>; runIndex: number }[];
  };
  if (server.runs.length !== p95Values.length) throw new Error("fixture_server_run_count");
  for (const [index, run] of server.runs.entries()) {
    const p95 = p95Values[index];
    if (p95 === undefined) throw new Error("fixture_server_p95_missing");
    run.measurements["p50Microseconds"] = p95;
    run.measurements["p95Microseconds"] = p95;
  }
  const aggregate = [...server.runs].sort(
    (left, right) =>
      (right.measurements["p95Microseconds"] as number) -
        (left.measurements["p95Microseconds"] as number) || left.runIndex - right.runIndex,
  )[0];
  if (aggregate === undefined) throw new Error("fixture_server_run_missing");
  server.measurements = structuredClone(aggregate.measurements);
}
...
describe("U4/16 evidence schema", () => {
  ...
  it("locks the exact workload, hard bounds, sample counts, and artifact binding", () => {
    ...
    expect(validated.server.methodology.independentRuns).toBe(1);
    expect(validated.server.runs).toHaveLength(1);
    expect(validated.server.runs[0]?.artifactSha256).toBe(SHA256);
    expect(validated.server.runs[0]?.environment).toEqual(validated.server.environment);
  });
  ...
  it("uses a permutation-invariant median server p95 reference", () => {
    const runs = [150, 100, 200].map((p95Microseconds, index) => ({
      measurements: { p95Microseconds },
      runIndex: index + 1,
    }));
    expect(medianServerRunP95(runs)).toBe(150);
    const [first, second, third] = runs;
    if (first === undefined || second === undefined || third === undefined) {
      throw new Error("fixture_server_run_missing");
    }
    expect(medianServerRunP95([third, first, second])).toBe(150);
  });
  ...
  it("fails hard bounds, artifact mismatch, regressions, and false qualification independently", () => {
    ...
    const mismatched = nestedMutation((value) => {
      ...
      for (const run of (value["server"] as { runs: { artifactSha256: string }[] }).runs) {
        run.artifactSha256 = "b".repeat(64);
      }
    });
  ...
  it.each(["artifact", "environment", "samples", "workload", "process"] as const)(
    "fails closed when one independent server process has mismatched %s evidence",
    (mutation) => {
      const value = evidence();
      qualifyThreeRuns(value);
      const runs = (value["server"] as { runs: Record<string, unknown>[] }).runs;
      const selected = runs[1];
      if (selected === undefined) throw new Error("fixture_server_run_missing");
      if (mutation === "artifact") selected["artifactSha256"] = "b".repeat(64);
      else if (mutation === "environment") {
        (selected["environment"] as Record<string, unknown>)["rustc"] = "mismatch";
      } else if (mutation === "samples") {
        (selected["methodology"] as Record<string, unknown>)["measuredSamples"] = 39;
      } else if (mutation === "workload") {
        (selected["workload"] as Record<string, unknown>)["files"] = 3;
      } else selected["processId"] = 10_001;
      expect(() => validateUploadBudgetEvidence(value)).toThrow("upload_budget_evidence_invalid");
    },
  );

  it("assembles a qualified server envelope only from three complete process results", () => {
    const value = evidence();
    qualifyThreeRuns(value);
    const server = value["server"] as {
      bounds: unknown;
    ...
    const processRuns = server.runs.map((run) => ({
      bounds: structuredClone(server.bounds),
      environment: structuredClone(run.environment),
      measurements: structuredClone(run.measurements),
      methodology: structuredClone(run.methodology),
      processId: run.processId,
      runIndex: run.runIndex,
      workload: structuredClone(run.workload),
    }));
    const assembled = uploadServerEvidenceFromProcessRuns(
      { processRuns, schemaVersion: 1 },
      SHA256,
      "qualified",
    );
    expect(assembled.runs.map(({ processId, runIndex }) => ({ processId, runIndex }))).toEqual([
      { processId: 10_001, runIndex: 1 },
      { processId: 10_002, runIndex: 2 },
      { processId: 10_003, runIndex: 3 },
    ]);
    expect(() =>
      uploadServerEvidenceFromProcessRuns(
        { processRuns: processRuns.slice(0, 2), schemaVersion: 1 },
        SHA256,
        "qualified",
      ),
    ).toThrow("upload_budget_evidence_invalid");
  });
  ...
  it("fails a skewed three-buffer server transfer even when the average remains two", () => {
    const mutated = nestedMutation((value) => {
      ...
      for (const measurements of [
        server.measurements,
        ...server.runs.map(({ measurements }) => measurements),
      ]) {
        const first = measurements.chunkBuffersByTransfer[0];
        const second = measurements.chunkBuffersByTransfer[1];
        if (first === undefined || second === undefined)
          throw new Error("fixture_transfer_missing");
  ...
  it("requires both environments and three independent browser and server runs for release qualification", () => {
  ...
  it.each([
    ...
    "reports %i of three exact server regressions and blocks only a repeated result",
    (regressedRunCount, confirmed) => {
      const baselineValue = evidence();
      qualifyThreeRuns(baselineValue);
      setServerRunP95s(baselineValue, [100, 100, 100]);
      const candidateValue = structuredClone(baselineValue);
      setServerRunP95s(
        candidateValue,
        [0, 1, 2].map((index) => (index < regressedRunCount ? 115 : 100)),
      );
      ...
      expect(evaluation.observations).toContain(
        `upload_budget:server:control_p95_regression_${String(regressedRunCount)}_of_3`,
      );
      expect(evaluation.issues.includes("upload_budget:server:control_p95_regression")).toBe(
        confirmed,
      );
    },
  );

  it("keeps the server regression classification invariant across run permutations", () => {
    const baselineValue = evidence();
    qualifyThreeRuns(baselineValue);
    setServerRunP95s(baselineValue, [90, 100, 110]);
    const candidateValue = structuredClone(baselineValue);
    setServerRunP95s(candidateValue, [115, 115, 115]);
    const baselineRuns = (baselineValue["server"] as { runs: Record<string, unknown>[] }).runs;
    const candidateRuns = (candidateValue["server"] as { runs: Record<string, unknown>[] }).runs;
    const permutations = <T>(values: readonly T[]): T[][] =>
      values.length === 0
        ? [[]]
        : values.flatMap((value, index) =>
    ...
    for (const baselinePermutation of permutations(baselineRuns)) {
      for (const candidatePermutation of permutations(candidateRuns)) {
        const permutedBaseline = structuredClone(baselineValue);
        (permutedBaseline["server"] as { runs: Record<string, unknown>[] }).runs =
          structuredClone(baselinePermutation);
        const permutedCandidate = structuredClone(candidateValue);
        (permutedCandidate["server"] as { runs: Record<string, unknown>[] }).runs =
          structuredClone(candidatePermutation);
        const evaluation = evaluateUploadBudget(
          validateUploadBudgetEvidence(permutedCandidate),
          validateUploadBudgetEvidence(permutedBaseline),
        ...
        outcomes.add(
          JSON.stringify({ issues: evaluation.issues, observations: evaluation.observations }),
        );
      }
    }
    expect(outcomes).toEqual(
      new Set([
        JSON.stringify({
          issues: ["upload_budget:server:control_p95_regression"],
          observations: ["upload_budget:server:control_p95_regression_3_of_3"],
        }),
      ]),
    );
  });
  ...
  it("applies hard latency caps to every independent server process run", () => {
    const value = evidence();
    qualifyThreeRuns(value);
    setServerRunP95s(value, [100, 2_001, 100]);
    const evaluation = evaluateUploadBudget(validateUploadBudgetEvidence(value), null);
    expect(evaluation.issues).toContain("upload_budget:server:run_2:control_p95_hard_cap");
  });

  it("applies manager resource caps to every independent server process run", () => {
    const value = evidence();
    qualifyThreeRuns(value);
    const measurements = (
      value["server"] as {
        runs: {
          measurements: {
            managerOwnedBytes: number;
            managerOwnedCategories: Record<string, number>;
            retainedBytes: number;
          };
        }[];
      }
    ).runs[1]?.measurements;
    if (measurements === undefined) throw new Error("fixture_server_run_missing");
    const increase = 600 * 1024;
    measurements.managerOwnedCategories["retainedHandleBytes"] = increase;
    measurements.managerOwnedBytes = 52_480 + increase;
    measurements.retainedBytes = 4 * (256 * 1024 + 64 * 1024) + measurements.managerOwnedBytes;
    const evaluation = evaluateUploadBudget(validateUploadBudgetEvidence(value), null);
    expect(evaluation.issues).toContain("upload_budget:server:run_2:manager_bytes_hard_cap");
  });

  it.each(["applicationValidation", "bodyIo", "provider", "scanner"] as const)(
    "rejects nonzero %s work in any independent server control run",
    (excludedPort) => {
      const value = evidence();
      qualifyThreeRuns(value);
      const excluded = (
        value["server"] as {
          runs: { measurements: { excludedCalls: Record<string, number> } }[];
        }
      ).runs[1]?.measurements.excludedCalls;
      if (excluded === undefined) throw new Error("fixture_server_run_missing");
      excluded[excludedPort] = 1;
      expect(() => validateUploadBudgetEvidence(value)).toThrow("upload_budget_evidence_invalid");
    },
  );

  it("does not report just-below-threshold server process noise as a regression", () => {
    const baselineValue = evidence();
    qualifyThreeRuns(baselineValue);
    setServerRunP95s(baselineValue, [100, 100, 100]);
    const candidateValue = structuredClone(baselineValue);
    setServerRunP95s(candidateValue, [114.999_999_9, 114.999_999_9, 114.999_999_9]);
    const evaluation = evaluateUploadBudget(
      validateUploadBudgetEvidence(candidateValue),
      validateUploadBudgetEvidence(baselineValue),
    );
    expect(evaluation.issues).not.toContain("upload_budget:server:control_p95_regression");
    expect(evaluation.observations).not.toContain(
      "upload_budget:server:control_p95_regression_3_of_3",
    );
  });
