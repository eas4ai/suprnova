...
describe("U4/16 evidence schema", () => {
  ...
  it.each([
    ...
    [
      ...
      (value: Record<string, unknown>) => {
        (
          value["browser"] as { measurements: { liveChunkBuffers: number } }
        ).measurements.liveChunkBuffers = 1;
      },
    ],
    [
      "unknown field",
  ...
  it("fails hard bounds, artifact mismatch, regressions, and false qualification independently", () => {
    ...
    const overCap = nestedMutation((value) => {
      (
        value["browser"] as { measurements: { progressP95Milliseconds: number } }
      ).measurements.progressP95Milliseconds = 17;
    });
    ...
    const regressed = nestedMutation((value) => {
      (
        value["browser"] as { measurements: { progressP95Milliseconds: number } }
      ).measurements.progressP95Milliseconds = 2.31;
    });
