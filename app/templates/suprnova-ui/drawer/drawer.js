// sn-drawer - hosts one native dialog used as a drawer. The dialog element owns
...
if (!customElements.get("sn-drawer")) {
      ...
      connectedCallback() {
        ...
        document.addEventListener(
          ...
          (event) => {
            ...
            const opener = target && target.closest(`[data-sn-drawer-open="${dialog.id}"]`);
            if (opener) {
              invoker = opener;
              if (!dialog.open) dialog.showModal();
              return;
            }
            const closer = target && target.closest(`[data-sn-drawer-close="${dialog.id}"]`);
