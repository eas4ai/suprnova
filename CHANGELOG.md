...
- **You choose where the Inertia error page middleware sits.** `Inertia::install` registers `InertiaErrorPageMiddleware` innermost of the Inertia layer, so it covers the handler, the route middleware, and everything you register after that call - which is why the scaffold puts `CsrfMiddleware` below it. It does not cover anything registered *above* the call, because a middleware that answers without calling `next` hands its response to nothing registered inside it. The case that bites is a lapsed session posting a form: `CsrfMiddleware` answers `419` with `{"message":"CSRF token mismatch."}`, and an app that registers CSRF above the install had no way to reach it, so the user got the Inertia crash modal on the one flow they are most likely to hit. An outer rate limiter's `429` and an auth guard's `401` were the same. Register `InertiaErrorPageMiddleware::new("Error")` yourself, after `SessionMiddleware` and `LocaleMiddleware` and before the middleware whose rejections it should cover, and `install` keeps your placement instead of adding a second. Naming a different component in the config than the one you registered now fails the install with an error naming both, rather than leaving the configured page silently unreachable.

### Fixed

- **An SSR page has one `<title>`, and it is the page's own.** The HTML shell wrote its `default_title` and then the SSR worker's head verbatim, so every page rendering a title through Inertia's `Head` component produced a document with two `<title>` elements and the framework's generic one first. First is the one the browser tab, the crawler and the link preview read, so the real title never showed. A worker head carrying a title now replaces the default rather than joining it; a head without one leaves the default exactly where it was.
...
- An app whose `CsrfMiddleware`, rate limiter, or auth guard is registered **before** `Inertia::install` should register `InertiaErrorPageMiddleware::new("Error")` after `LocaleMiddleware` and before that middleware, so its rejections render the error page instead of reaching the client as raw JSON. Keep the `.error_page("Error")` on the config as well - it must name the same component, or the install now fails - and `install` will skip adding its own. The scaffolded `bootstrap.rs` registers CSRF after the install, so a project generated by `suprnova new` needs no change.
- An app that renders its own `<title>` through Inertia's `Head` component under SSR will see `default_title` stop appearing in the document. That is the fix; if you were relying on the default as a suffix, put it in the `Head` component where the rest of the title lives.
