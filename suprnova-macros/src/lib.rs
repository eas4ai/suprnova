...
///
/// Walks the impl block at parse time, identifies which of the 16
/// lifecycle methods the user overrode by name match, and emits one
/// adapter listener per overridden method. The adapters delegate into
/// the user's trait impl through `EventFacade::listen` (non-cancellable
/// events) and `listen_cancellable` (cancellable events). Registration
/// is wrapped in an idempotent `install` fn whose pointer is submitted
/// to the `ObserverEntry` inventory T2a ships; `bootstrap_observers`
/// drains that inventory once at boot.
///
...
///
/// ```rust,ignore
/// #[suprnova::observer(User)]   // <- outer, runs first
/// #[async_trait]                 // <- inner, rewrites the impl items
...
/// `async fn` into the desugared `Pin<Box<dyn Future>>` poll-fn shape;
/// if `#[async_trait]` runs first, this macro's name-match against the
/// 16 trait method names finds nothing and silently emits zero
/// listeners.
///
/// # Limitations
///
/// The observer struct must be a unit struct in v1. The macro
/// constructs the observer via `let obs = SomeObserver;`, which only
/// works for zero-sized types. Stateful observers
/// (e.g. `Arc<Inner>::clone()`) are a v2 concern; the current target
/// matches Laravel's `Observable::observe(SomeObserver::class)` shape.
///
/// # Idempotency
///
