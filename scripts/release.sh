...
#   2. Bumps and commits all release metadata.
#   3. Runs the canonical full local gate on that exact clean release commit.
...
release_exit_cleanup() {
  local status=$?
  local current_head parent subject stamp_path
  trap - EXIT
  if [[ $status -ne 0 && $ROLLBACK_ARMED -eq 1 && -n "$ORIGINAL_HEAD" ]]; then
    current_head="$(git rev-parse HEAD)"
    if [[ "$current_head" == "$ORIGINAL_HEAD" ]]; then
      git reset --hard "$ORIGINAL_HEAD" >/dev/null
    else
      parent="$(git rev-parse "$current_head^" 2>/dev/null || true)"
      subject="$(git log -1 --format=%s "$current_head" 2>/dev/null || true)"
      if [[ "$parent" == "$ORIGINAL_HEAD" && "$subject" == "release: v$NEW_VERSION" ]]; then
        git reset --hard "$ORIGINAL_HEAD" >/dev/null
      else
        echo "error: refusing release rollback because HEAD is not the script-owned release commit" >&2
      fi
    fi
    stamp_path="$(git rev-parse --git-path suprnova-gate-pass)"
    rm -f "$stamp_path"
    echo "==> rolled back release-owned commit and files to $ORIGINAL_HEAD" >&2
  fi
...
# ---------- 2. Bump workspace version metadata -----------------------------
ROLLBACK_ARMED=1

echo "==> bumping workspace version metadata to $NEW_VERSION"
BUMPED_MANIFEST_OUTPUT=""
if ! BUMPED_MANIFEST_OUTPUT="$(
  python3 scripts/bump-workspace-version.py "$NEW_VERSION"
)"; then
  echo "error: version helper failed; release commit, tag, and push were not created" >&2
  exit 1
fi
BUMPED_MANIFESTS=()
if [[ -n "$BUMPED_MANIFEST_OUTPUT" ]]; then
  mapfile -t BUMPED_MANIFESTS <<<"$BUMPED_MANIFEST_OUTPUT"
fi

# The version helper updates tagged commands in every manual mirror and the
# English "tagged release" sentence. Update the same current-release sentence
# in each localized installation chapter, then refresh the translation ledger
# before the release commit is created.
LOCALIZED_INSTALLATIONS="$(
  python3 - "$CURRENT_VERSION" "$NEW_VERSION" <<'PY'
from pathlib import Path
import re
import sys

_, proposed = sys.argv[1:]
tagged_version = re.compile(r"v[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?")
for locale in ("de", "es", "fr", "ja", "pt-BR", "zh-Hans"):
    path = Path("manual") / locale / "installation.md"
    lines = path.read_text().splitlines(keepends=True)
    for index, line in enumerate(lines):
        if "Rust 1.94.0" not in line:
            continue
        lines[index], count = tagged_version.subn(f"v{proposed}", line, count=1)
        if count == 1:
            break
    else:
        raise SystemExit(f"{path} has no tagged release in its current-main Rust requirement")
    path.write_text("".join(lines))
    print(path)
PY
)"
mapfile -t LOCALIZED_INSTALLATION_FILES <<<"$LOCALIZED_INSTALLATIONS"
BUMPED_MANIFESTS+=("${LOCALIZED_INSTALLATION_FILES[@]}")
# The version helper rewrites tagged versions in every chapter that pins one
# (installation, cli, cli-new, broadcasting, filesystem, payments, testing,
# and whatever it learns next), in English and in every locale mirror, so
# the ledger has to be restamped for exactly the chapters it reported -
# stamping installation.md alone leaves the rest STALE and the full gate
# fails on translation-lock after the release commit exists.
STAMP_CHAPTERS=(installation.md)
for bumped in "${BUMPED_MANIFESTS[@]}"; do
  case "$bumped" in
    manual/*/*) ;;
    manual/*.md)
      chapter="$(basename "$bumped")"
      [[ "$chapter" == "installation.md" ]] || STAMP_CHAPTERS+=("$chapter")
      ;;
  esac
done
scripts/check-manual-translations.sh --stamp "${STAMP_CHAPTERS[@]}"
BUMPED_MANIFESTS+=(.manual-translations.lock)
...
# Refresh Cargo.lock so the release commit is self-contained.
echo "==> cargo check --workspace (refresh Cargo.lock for the bumped version)"
cargo check --workspace

# ---------- 3. Commit and run canonical full gate --------------------------

echo "==> committing release: v$NEW_VERSION"
git add Cargo.lock "${BUMPED_MANIFESTS[@]}"
git commit -m "release: v$NEW_VERSION"

echo "==> canonical full gate on release commit $(git rev-parse HEAD)"
