...
tree_has_package_version_prefix() {
    local tree=$1 package=$2 prefix=$3
    awk -v package="$package" -v prefix="v$prefix" \
        '$1 == package && index($2, prefix) == 1 { found = 1 } END { exit !found }' \
        "$tree"
}

assert_supported_database_stack() {
    local tree=$1
    for requirement in 'sea-orm 2.' 'sea-query 1.' 'sqlx 0.9' 'sqlx-core 0.9'; do
        set -- $requirement
        if ! tree_has_package_version_prefix "$tree" "$1" "$2"; then
            echo "unsupported database dependency in $(basename "$tree"): expected $1 v$2" >&2
            exit 1
        fi
    done
    for forbidden in 'sea-orm 1.' 'sea-query 0.' 'sqlx 0.8' 'sqlx-core 0.8' \
        'sqlx-mysql 0.8' 'sqlx-postgres 0.8' 'sqlx-sqlite 0.8'; do
        set -- $forbidden
        if tree_has_package_version_prefix "$tree" "$1" "$2"; then
            echo "unsupported database dependency in $(basename "$tree"): $1 v$2" >&2
            exit 1
        fi
    done
}
...
run "zero-driver rustdoc links" \
    env RUSTC_BOOTSTRAP=1 cargo rustdoc -p suprnova --lib \
        --no-default-features -- -Z unstable-options --output-format json
...
#
# These feature-isolation checks use rustdoc's JSON renderer. It executes
# the same rustdoc lints (the gate's broken-link canary proves this) without
# HTML's blanket-implementation and search-index pass, which can exceed
# 60 GiB RSS for this crate. The release gate uses the same canary-proven
# renderer for the default feature set.
run "filesystem-only rustdoc links" \
    env RUSTC_BOOTSTRAP=1 cargo rustdoc -p suprnova --lib \
        --no-default-features --features filesystem -- \
        -Z unstable-options --output-format json
run "filesystem+azure+gcs rustdoc links" \
    env RUSTC_BOOTSTRAP=1 cargo rustdoc -p suprnova --lib \
        --no-default-features --features filesystem-azure,filesystem-gcs -- \
        -Z unstable-options --output-format json
...
# the first configuration to lint the localization module with nothing else
# on - a doc link from `Lang` or the middleware to a `testing`-gated or
# driver-gated item resolves in every other build and breaks only here.
run "localization-only rustdoc links" \
    env RUSTC_BOOTSTRAP=1 cargo rustdoc -p suprnova --lib \
        --no-default-features --features localization -- \
        -Z unstable-options --output-format json
...
run "default rustdoc links" \
    env RUSTC_BOOTSTRAP=1 cargo rustdoc -p suprnova --lib -- \
        -Z unstable-options --output-format json
run "all-features profile" \
    cargo check -p suprnova --all-features
run "all-features rustdoc links" \
    env RUSTC_BOOTSTRAP=1 cargo rustdoc -p suprnova --lib --all-features -- \
        -Z unstable-options --output-format json
...
for tree in sqlite postgres mysql minimal default all-features workspace-all-targets; do
    assert_supported_database_stack "$TMP_DIR/$tree.tree"
done
...
assert_version_present "$TMP_DIR/sqlite.tree" sqlx-sqlite v0.9
assert_version_present "$TMP_DIR/postgres.tree" sqlx-postgres v0.9
assert_version_present "$TMP_DIR/mysql.tree" sqlx-mysql v0.9
